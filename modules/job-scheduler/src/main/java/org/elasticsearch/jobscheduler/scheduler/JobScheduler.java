...
import org.elasticsearch.jobscheduler.JobSchedulerPlugin;
import org.elasticsearch.jobscheduler.spi.JobDocVersion;
...
import org.elasticsearch.jobscheduler.utils.VisibleForTesting;
...
public class JobScheduler {
    ...
    boolean reschedule(ScheduledJobParameter jobParameter, JobSchedulingInfo jobInfo, ScheduledJobRunner jobRunner,
        ...
        Instant instant0 = this.clock.instant();
        Duration duration = Duration.between(instant0, nextExecutionTime);
        if (duration.getSeconds() < 0) {
            log.warn("reschedule: {} {} {} {}",
                jobParameter.getName(), duration.getSeconds(), nextExecutionTime, instant0);
            nextExecutionTime = instant0;
            duration = Duration.between(instant0, nextExecutionTime);
            log.warn("nextExecutionTime illegal: set to {} duration {}", nextExecutionTime, duration.getSeconds());
        }
