...
public class WhitelistApiTest extends AbstractRestApiUnitTest {
    ...
    /**
     * admin_superuser is a user who has all permissions - essentially an admin user, not the same as superadmin.
     * superadmin is identified by a certificate that should be passed as a part of the request header.
     */
    private final Header adminCredsHeader = encodeBasicHeader("admin_superuser", "admin_superuser");
    ...
    private void testGetAndPut(final int expectedStatus, final boolean sendAdminCertificate, final Header... headers) throws Exception {
        ...
        //CHECK GET REQUEST
        response = rh.executeGetRequest("_security/whitelist", headers);
        ...
        //CHECK PUT REQUEST
        response = rh.executePutRequest("_security/whitelist", "{\"enabled\": true, \"requests\": {\"/_cat/nodes\": [\"GET\"],\"/_cat/indices\": [\"GET\"] }}", headers);
        assertThat(response.getBody(), response.getStatusCode(), equalTo(expectedStatus));
    ...
    public void testResponseDoesNotContainMetaHeader() throws Exception {
        ...
        RestHelper.HttpResponse response = rh.executeGetRequest("_security/whitelist");
    ...
    public void testPutUnknownKey() throws Exception {
        ...
        rh.keystore = "restapi/kirk-keystore.jks";
        rh.sendAdminCertificate = true;
        RestHelper.HttpResponse response = rh.executePutRequest("_security/whitelist", "{ \"unknownkey\": true, \"requests\": {\"/_cat/nodes\": [\"GET\"],\"/_cat/indices\": [\"GET\"] }}");
    ...
    public void testPutInvalidJson() throws Exception {
        ...
        rh.keystore = "restapi/kirk-keystore.jks";
        rh.sendAdminCertificate = true;
        RestHelper.HttpResponse response = rh.executePutRequest("_security/whitelist", "{ \"invalid\"::{{ [\"*\"], \"requests\": {\"/_cat/nodes\": [\"GET\"],\"/_cat/indices\": [\"GET\"] }}");
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
        assertHealthy();
    }
    ...
    public void testPayloadMandatory() throws Exception {
        ...
        rh.keystore = "restapi/kirk-keystore.jks";
        rh.sendAdminCertificate = true;
        response = rh.executePutRequest("/_security/whitelist", "", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
    ...
    public void testWhitelistInvalidHttpRequestMethod() throws Exception{
        ...
        response = rh.executePutRequest("_security/whitelist", "{\"enabled\": true, \"requests\": {\"/_cat/nodes\": [\"GE\"],\"/_cat/indices\": [\"PUT\"] }}", adminCredsHeader);
    ...
    public void testPatchApi() throws Exception{
        ...
        response = rh.executePatchRequest("_security/whitelist", "[{ \"op\": \"replace\", \"path\": \"/config\", \"value\": {\"enabled\": true, \"requests\": {\"/_cat/nodes\": [\"GET\"],\"/_cat/indices\": [\"PUT\"] }}}]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        response = rh.executeGetRequest("_security/whitelist", adminCredsHeader);
        assertEquals(response.getBody(),"{\"config\":{\"enabled\":true,\"requests\":{\"/_cat/nodes\":[\"GET\"],\"/_cat/indices\":[\"PUT\"]}}}");

        //PATCH just requests
        response = rh.executePatchRequest("_security/whitelist", "[{ \"op\": \"replace\", \"path\": \"/config/requests\", \"value\": {\"/_cat/nodes\": [\"GET\"]}}]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        response = rh.executeGetRequest("_security/whitelist", adminCredsHeader);
        ...
        response = rh.executePatchRequest("_security/whitelist", "[{ \"op\": \"replace\", \"path\": \"/config/enabled\", \"value\": false}]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        response = rh.executeGetRequest("_security/whitelist", adminCredsHeader);
        ...
        response = rh.executePatchRequest("_security/whitelist", "[{ \"op\": \"add\", \"path\": \"/config/enabled\", \"value\": true}]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        response = rh.executeGetRequest("_security/whitelist", adminCredsHeader);
        ...
        response = rh.executePatchRequest("_security/whitelist", "[{ \"op\": \"add\", \"path\": \"/config/enabled\", \"value\": false}]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());response = rh.executeGetRequest("_security/whitelist", adminCredsHeader);
        response = rh.executeGetRequest("_security/whitelist", adminCredsHeader);
        assertTrue(response.getBody().contains("\"enabled\":false"));
    }
}
