...
public abstract class AbstractRestApiUnitTest extends SingleClusterTest {
	...
	protected final void setup() throws Exception {
		...
		builder.put("security.ssl.http.enabled", true)
				.put("security.ssl.http.keystore_filepath",
						FileHelper.getAbsoluteFilePathFromClassPath("restapi/node-0-keystore.jks"))
				.put("security.ssl.http.truststore_filepath",
						FileHelper.getAbsoluteFilePathFromClassPath("restapi/truststore.jks"));

        builder.put("security.restapi.roles_enabled", "superuser,security_rest_api_access");
	...
	protected final void setupWithRestRoles(Settings nodeOverride) throws Exception {
		...
		builder.put("security.restapi.roles_enabled.0", "security_role_klingons");
		builder.put("security.restapi.roles_enabled.1", "security_role_vulcans");
		builder.put("security.restapi.roles_enabled.2", "security_test");

		builder.put("security.restapi.endpoints_disabled.global.CACHE.0", "*");

		builder.put("security.restapi.endpoints_disabled.security_role_klingons.conFiGuration.0", "*");
		builder.put("security.restapi.endpoints_disabled.security_role_klingons.wRongType.0", "WRONGType");
		builder.put("security.restapi.endpoints_disabled.security_role_klingons.MAPPING.0", "PUT");
		builder.put("security.restapi.endpoints_disabled.security_role_klingons.MAPPING.1", "DELETE");

		builder.put("security.restapi.endpoints_disabled.security_role_vulcans.CONFIG.0", "*");
	...
	protected void deleteUser(String username) throws Exception {
		...
		RestHelper.HttpResponse response = rh.executeDeleteRequest("/_security/user/" + username, new Header[0]);
	...
	protected void addUserWithPassword(String username, String password, int status) throws Exception {
		boolean sendAdminCertificate = rh.sendAdminCertificate;
		rh.sendAdminCertificate = true;
		RestHelper.HttpResponse response = rh.executePutRequest("/_security/user/" + username,
	...
	protected void addUserWithPassword(String username, String password, String[] roles, int status) throws Exception {
		boolean sendAdminCertificate = rh.sendAdminCertificate;
		rh.sendAdminCertificate = true;
		String payload = "{" + "\"password\": \"" + password + "\"," + "\"external_roles\": [";
		...
		payload += "]}";
		RestHelper.HttpResponse response = rh.executePutRequest("/_security/user/" + username, payload, new Header[0]);
		Assert.assertEquals(status, response.getStatusCode());
		rh.sendAdminCertificate = sendAdminCertificate;
	}
	...
	protected void addUserWithHash(String username, String hash, int status) throws Exception {
		boolean sendAdminCertificate = rh.sendAdminCertificate;
		rh.sendAdminCertificate = true;
		RestHelper.HttpResponse response = rh.executePutRequest("/_security/user/" + username, "{\"hash\": \"" + hash + "\"}",
				new Header[0]);
		Assert.assertEquals(status, response.getStatusCode());
		rh.sendAdminCertificate = sendAdminCertificate;
	}
	...
	protected void addUserWithPasswordAndHash(String username, String password, String hash, int status) throws Exception {
		boolean sendAdminCertificate = rh.sendAdminCertificate;
		rh.sendAdminCertificate = true;
		RestHelper.HttpResponse response = rh.executePutRequest("/_security/user/" + username, "{\"hash\": \"" + hash + "\", \"password\": \"" + password + "\"}",
				new Header[0]);
		Assert.assertEquals(status, response.getStatusCode());
		rh.sendAdminCertificate = sendAdminCertificate;
	}
	...
	protected void assertHealthy() throws Exception {
		//Assert.assertEquals(HttpStatus.SC_OK, rh.executeGetRequest("_security/health?pretty").getStatusCode());
		Assert.assertEquals(HttpStatus.SC_OK, rh.executeGetRequest("_security/authinfo?pretty", encodeBasicHeader("admin", "admin")).getStatusCode());
