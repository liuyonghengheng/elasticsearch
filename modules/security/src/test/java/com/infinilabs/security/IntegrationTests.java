...
public class IntegrationTests extends SingleClusterTest {
    ...
    public void testDNSpecials1() throws Exception {
        ...
        final Settings settings = Settings.builder()
                ...
                .put(SSLConfigConstants.SECURITY_SSL_TRANSPORT_KEYSTORE_ALIAS, "1")
                .put(SSLConfigConstants.SECURITY_SSL_TRANSPORT_KEYSTORE_TYPE, "PKCS12")
                .putList("security.nodes_dn", "EMAILADDRESS=unt@tst.com,CN=node-untspec5.example.com,OU=SSL,O=Te\\, st,L=Test,C=DE")
                .putList("security.authcz.admin_dn", "EMAILADDREss=unt@xxx.com,  cn=node-untspec6.example.com, OU=SSL,O=Te\\, st,L=Test, c=DE")
                .put("security.cert.oid","1.2.3.4.5.6")
                .build();


        Settings tcSettings = Settings.builder()
                .put("security.ssl.transport.keystore_filepath", FileHelper.getAbsoluteFilePathFromClassPath("node-untspec6-keystore.p12"))
                .put(SSLConfigConstants.SECURITY_SSL_TRANSPORT_KEYSTORE_TYPE, "PKCS12")
                .build();
    ...
    public void testRestImpersonation() throws Exception {
        ...
        HttpResponse resp;
        resp = rh.executeGetRequest("/_security/authinfo", new BasicHeader("security_run_as", "knuddel"), encodeBasicHeader("worf", "worf"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, resp.getStatusCode());

        resp = rh.executeGetRequest("/_security/authinfo", new BasicHeader("security_run_as", "knuddel"), encodeBasicHeader("spock", "spock"));
        ...
        resp = rh.executeGetRequest("/_security/authinfo", new BasicHeader("security_run_as", "userwhonotexists"), encodeBasicHeader("spock", "spock"));
        System.out.println(resp.getBody());
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, resp.getStatusCode());

        resp = rh.executeGetRequest("/_security/authinfo", new BasicHeader("security_run_as", "invalid"), encodeBasicHeader("spock", "spock"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, resp.getStatusCode());
    }
    ...
    public void testXff() throws Exception {
        ...
        RestHelper rh = nonSslRestHelper();
        HttpResponse resc = rh.executeGetRequest("_security/authinfo", new BasicHeader("x-forwarded-for", "10.0.0.7"), encodeBasicHeader("worf", "worf"));
    ...
    public void testMultiRoleSpan() throws Exception {
        ...
        try (TransportClient tc = getInternalTransportClient()) {
            tc.index(new IndexRequest(".security").type(getType()).id("config").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("config", FileHelper.readYamlContent("config_multirolespan.yml"))).actionGet();
    ...
    public void testSecurityIndexSecurity() throws Exception {
        ...
        HttpResponse res = rh.executePutRequest(".security/_mapping?pretty", "{\"properties\": {\"name\":{\"type\":\"text\"}}}",
        ...
        res = rh.executePutRequest("_all/_mapping?pretty", "{\"properties\": {\"name\":{\"type\":\"text\"}}}",
                encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());
        res = rh.executePostRequest(".security/_close", "",
                encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());
        res = rh.executeDeleteRequest(".security",
                encodeBasicHeader("nagilum", "nagilum"));
        res = rh.executeDeleteRequest("_all",
                encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());
        res = rh.executePutRequest(".security/_settings", "{\"index\" : {\"number_of_replicas\" : 2}}",
                encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());
        res = rh.executePutRequest(".opendistro_secur*/_settings", "{\"index\" : {\"number_of_replicas\" : 2}}",
                encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());
//        res = rh.executePostRequest(".security/_freeze", "",
//                encodeBasicHeader("nagilum", "nagilum"));
//        Assert.assertTrue(res.getStatusCode() >= 400);

        String bulkBody = "{ \"index\" : { \"_index\" : \".security\", \"_id\" : \"1\" } }\n"
                + "{ \"field1\" : \"value1\" }\n"
                + "{ \"index\" : { \"_index\" : \".security\", \"_id\" : \"2\" } }\n"
                + "{ \"field2\" : \"value2\" }\n"
                + "{ \"index\" : { \"_index\" : \"myindex\", \"_id\" : \"2\" } }\n"
                + "{ \"field2\" : \"value2\" }\n"
                + "{ \"delete\" : { \"_index\" : \".security\", \"_id\" : \"config\" } }\n";
