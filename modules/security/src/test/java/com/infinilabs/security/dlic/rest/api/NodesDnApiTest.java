...
public class NodesDnApiTest extends AbstractRestApiUnitTest {
    ...
    private void testCrudScenarios(final int expectedStatus, final Header... headers) throws Exception {
        response = rh.executeGetRequest("_security/nodesdn?show_all=true", headers);
        ...
        response = rh.executeGetRequest("_security/nodesdn?show_all=false", headers);
        ...
        response = rh.executeGetRequest("_security/nodesdn", headers);
        assertThat(response.getBody(), response.getStatusCode(), equalTo(expectedStatus));
        if (expectedStatus == HttpStatus.SC_OK) {
            JsonNode expected = asJsonNode(ImmutableMap.of("cluster1", nodesDnEntry("cn=popeye")));
            JsonNode node = OBJECT_MAPPER.readTree(response.getBody());
        ...
        response = rh.executeGetRequest("_security/nodesdn/cluster1", headers);
        assertThat(response.getBody(), response.getStatusCode(), equalTo(expectedStatus));
        if (expectedStatus == HttpStatus.SC_OK) {
            JsonNode expected = asJsonNode(ImmutableMap.of("cluster1", nodesDnEntry("cn=popeye")));
            JsonNode node = OBJECT_MAPPER.readTree(response.getBody());
        ...
        response = rh.executePutRequest("_security/nodesdn/cluster1", "{\"nodes_dn\": [\"cn=popeye\"]}", headers);
        assertThat(response.getBody(), response.getStatusCode(), equalTo(expectedStatus));

        response = rh.executePatchRequest("/_security/nodesdn/cluster1", "[{ \"op\": \"add\", \"path\": \"/nodes_dn/-\", \"value\": \"bluto\" }]", headers);
        assertThat(response.getBody(), response.getStatusCode(), equalTo(expectedStatus));

        response = rh.executePatchRequest("/_security/nodesdn", "[{ \"op\": \"remove\", \"path\": \"/cluster1/nodes_dn/0\"}]", headers);
        assertThat(response.getBody(), response.getStatusCode(), equalTo(expectedStatus));

        response = rh.executeDeleteRequest("_security/nodesdn/cluster1", headers);
        assertThat(response.getBody(), response.getStatusCode(), equalTo(expectedStatus));
    }
    ...
    public void testNodesDnApi() throws Exception {
...

        {
            ...
            response = rh.executePutRequest("_security/nodesdn/" + NodesDnApiAction.STATIC_ES_YML_NODES_DN, "{\"nodes_dn\": [\"cn=popeye\"]}", nonAdminCredsHeader);
            assertThat(response.getBody(), response.getStatusCode(), equalTo(expectedStatus));

            response = rh.executePatchRequest("/_security/nodesdn/" + NodesDnApiAction.STATIC_ES_YML_NODES_DN,
                "[{ \"op\": \"add\", \"path\": \"/nodes_dn/-\", \"value\": \"bluto\" }]" , nonAdminCredsHeader);
            assertThat(response.getBody(), response.getStatusCode(), equalTo(expectedStatus));

            response = rh.executeDeleteRequest("_security/nodesdn/" + NodesDnApiAction.STATIC_ES_YML_NODES_DN, nonAdminCredsHeader);
            assertThat(response.getBody(), response.getStatusCode(), equalTo(expectedStatus));
        }
    }
