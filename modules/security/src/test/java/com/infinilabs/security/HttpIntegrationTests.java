...
import static com.infinilabs.security.DefaultObjectMapper.readTree;
...
public class HttpIntegrationTests extends SingleClusterTest {
    ...
    public void testHTTPBasic() throws Exception {
        ...
        final RestHelper rh = nonSslRestHelper();
            ...
            Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executePutRequest(".security/config/2", "{}",encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
            Assert.assertEquals(HttpStatus.SC_NOT_FOUND, rh.executeGetRequest(".security/config/0", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
            ...
            Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executeDeleteRequest(".security/", encodeBasicHeader("worf", "worf")).getStatusCode());
            Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executePostRequest("/.security/_close", null,encodeBasicHeader("worf", "worf")).getStatusCode());
            Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executePostRequest("/.security/_upgrade", null,encodeBasicHeader("worf", "worf")).getStatusCode());
            Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executePutRequest("/.security/_mapping","{}",encodeBasicHeader("worf", "worf")).getStatusCode());

            Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executeGetRequest(".security/", encodeBasicHeader("worf", "worf")).getStatusCode());
            Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executePutRequest(".security/config/2", "{}",encodeBasicHeader("worf", "worf")).getStatusCode());
            Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executeGetRequest(".security/config/0",encodeBasicHeader("worf", "worf")).getStatusCode());
            Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executeDeleteRequest(".security/config/0",encodeBasicHeader("worf", "worf")).getStatusCode());
            Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executePutRequest(".security/config/0","{}",encodeBasicHeader("worf", "worf")).getStatusCode());
    ...
    //all
            ...
            Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executePostRequest(".security/_mget","{\"ids\" : [\"0\"]}",encodeBasicHeader("worf", "worf")).getStatusCode());

            Assert.assertEquals(HttpStatus.SC_OK, rh.executeGetRequest("starfleet/ships/_search?pretty", encodeBasicHeader("worf", "worf")).getStatusCode());

            try (TransportClient tc = getInternalTransportClient()) {
                tc.index(new IndexRequest(".security").type(getType()).id("roles").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("roles", FileHelper.readYamlContent("roles_deny.yml"))).actionGet();
            ...
            try (TransportClient tc = getInternalTransportClient()) {
                tc.index(new IndexRequest(".security").type(getType()).id("roles").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("roles", FileHelper.readYamlContent("role.yml"))).actionGet();
                ConfigUpdateResponse cur = tc.execute(ConfigUpdateAction.INSTANCE, new ConfigUpdateRequest(new String[]{"roles"})).actionGet();
                Assert.assertEquals(clusterInfo.numNodes, cur.getNodes().size());
            }
            ...
            Assert.assertTrue(!res.getBody().contains(".security"));
            ...
            res = rh.executeGetRequest("_security/authinfo", new BasicHeader("security_tenant", "unittesttenant"), encodeBasicHeader("worf", "worf"));
            Assert.assertEquals(HttpStatus.SC_OK, res.getStatusCode());
            ...
            Assert.assertTrue(res.getBody().contains("\"username\":\"worf\""));

            res = rh.executeGetRequest("_security/authinfo", encodeBasicHeader("worf", "worf"));
            Assert.assertEquals(HttpStatus.SC_OK, res.getStatusCode());
            Assert.assertTrue(res.getBody().contains("tenant"));
            Assert.assertTrue(res.getBody().contains("\"user_requested_tenant\":null"));
            Assert.assertTrue(res.getBody().contains("\"kltentrw\":true"));
            Assert.assertTrue(res.getBody().contains("\"username\":\"worf\""));
            Assert.assertTrue(res.getBody().contains("\"attributes\":[]"));
            Assert.assertFalse(res.getBody().contains("attributes="));

            res = rh.executeGetRequest("_security/authinfo?pretty", encodeBasicHeader("custattr", "nagilum"));
            Assert.assertEquals(HttpStatus.SC_OK, res.getStatusCode());
            Assert.assertTrue(res.getBody().contains("tenants"));
            Assert.assertTrue(res.getBody().contains("\"user_requested_tenant\" : null"));
            Assert.assertTrue(res.getBody().contains("\"username\" : \"custattr\""));
            Assert.assertTrue(res.getBody().contains("\"attributes\" : ["));
            ...
            //rest impersonation
            res = rh.executeGetRequest("_security/authinfo", new BasicHeader("security_run_as","knuddel"), encodeBasicHeader("worf", "worf"));
            Assert.assertEquals(HttpStatus.SC_OK, res.getStatusCode());
            ...
            res = rh.executeGetRequest("_security/authinfo", new BasicHeader("security_run_as","nonexists"), encodeBasicHeader("worf", "worf"));
            Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());

            res = rh.executeGetRequest("_security/authinfo", new BasicHeader("security_run_as","notallowed"), encodeBasicHeader("worf", "worf"));
            Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());
        }
    ...
    public void testHTTPAnon() throws Exception {
            ...
            HttpResponse resc = rh.executeGetRequest("_security/authinfo");
            Assert.assertTrue(resc.getBody().contains("security_anonymous"));
            Assert.assertEquals(HttpStatus.SC_OK, resc.getStatusCode());

            resc = rh.executeGetRequest("_security/authinfo?pretty=true");
            System.out.println(resc.getBody());
            Assert.assertTrue(resc.getBody().contains("\"remote_address\" : \"")); //check pretty print
            Assert.assertEquals(HttpStatus.SC_OK, resc.getStatusCode());

            resc = rh.executeGetRequest("_security/authinfo", encodeBasicHeader("nagilum", "nagilum"));
            System.out.println(resc.getBody());
            Assert.assertTrue(resc.getBody().contains("nagilum"));
            Assert.assertFalse(resc.getBody().contains("security_anonymous"));
            Assert.assertEquals(HttpStatus.SC_OK, resc.getStatusCode());

            try (TransportClient tc = getInternalTransportClient()) {
                tc.index(new IndexRequest(".security").type(getType()).id("config").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("config", FileHelper.readYamlContent("config.yml"))).actionGet();
                tc.index(new IndexRequest(".security").type(getType()).setRefreshPolicy(RefreshPolicy.IMMEDIATE).id("user").source("user", FileHelper.readYamlContent("user.yml"))).actionGet();
            ...
            Assert.assertEquals(HttpStatus.SC_UNAUTHORIZED, rh.executeGetRequest("").getStatusCode());
            Assert.assertEquals(HttpStatus.SC_UNAUTHORIZED, rh.executeGetRequest("_security/authinfo").getStatusCode());
            Assert.assertEquals(HttpStatus.SC_UNAUTHORIZED, rh.executeGetRequest("", encodeBasicHeader("worf", "wrong")).getStatusCode());
            Assert.assertEquals(HttpStatus.SC_OK, rh.executeGetRequest("", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
    }
    ...
    public void testHTTPClientCert() throws Exception {
        final Settings settings = Settings.builder()
                .put("security.ssl.http.clientauth_mode","REQUIRE")
                .put("security.ssl.http.enabled",true)
                .put("security.ssl.http.keystore_filepath", FileHelper.getAbsoluteFilePathFromClassPath("node-0-keystore.jks"))
                .put("security.ssl.http.truststore_filepath", FileHelper.getAbsoluteFilePathFromClassPath("truststore.jks"))
        ...
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executePutRequest(".security/"+getType()+"/x", "{}").getStatusCode());

        rh.keystore = "kirk-keystore.jks";
        Assert.assertEquals(HttpStatus.SC_CREATED, rh.executePutRequest(".security/"+getType()+"/y", "{}").getStatusCode());
        HttpResponse res;
        Assert.assertEquals(HttpStatus.SC_OK, (res = rh.executeGetRequest("_security/authinfo")).getStatusCode());
        System.out.println(res.getBody());
    }
    ...
    public void testHTTPProxyRolesSeparator() throws Exception {
        ...
        HttpResponse res = rh.executeGetRequest("_security/authinfo", new BasicHeader("x-forwarded-for", "localhost,192.168.0.1,10.0.0.2"),new BasicHeader("user", "scotty"),new BasicHeader("roles", "starfleet,engineer"));
        Assert.assertTrue("Expected one backend role since separator is incorrect", res.getBody().contains("\"external_roles\":[\"starfleet,engineer\"]"));
        // correct separator, now we should see two backend roles
        res = rh.executeGetRequest("_security/authinfo", new BasicHeader("x-forwarded-for", "localhost,192.168.0.1,10.0.0.2"),new BasicHeader("user", "scotty"),new BasicHeader("roles", "starfleet;engineer"));
        Assert.assertTrue("Expected two backend roles string since separator is correct: " + res.getBody(), res.getBody().contains("\"external_roles\":[\"starfleet\",\"engineer\"]"));
    ...
    public void testTenantInfo() throws Exception {
        ...
        HttpResponse res = rh.executeGetRequest("_security/tenantinfo?pretty", encodeBasicHeader("itt1635", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());
    ...
    public void testRestImpersonation() throws Exception {
        final Settings settings = Settings.builder()
                .putList(ConfigConstants.SECURITY_AUTHCZ_REST_IMPERSONATION_USERS+".worf", "someotherusernotinuserfile")
                .build();
        setup(Settings.EMPTY, new DynamicSecurityConfig().setConfig("config_rest_impersonation.yml"), settings);
        final RestHelper rh = nonSslRestHelper();

        //rest impersonation
        HttpResponse res = rh.executeGetRequest("_security/authinfo", new BasicHeader("security_run_as","someotherusernotinuserfile"), encodeBasicHeader("worf", "worf"));
        Assert.assertEquals(HttpStatus.SC_OK, res.getStatusCode());
        Assert.assertTrue(res.getBody().contains("name=someotherusernotinuserfile"));
        Assert.assertFalse(res.getBody().contains("worf"));
    }
