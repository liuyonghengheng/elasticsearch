...
import com.infinilabs.security.test.helper.rest.EasyRestHelper;
...
public class OpenDistroSecuritySSLCertsInfoActionTests extends SingleClusterTest {
    ...
    private final String ENDPOINT = "_security/ssl/certs";
    ...
    @Test
    public void testCertCluster() throws Exception {
        //测试集群启用证书后节点通信是否正常
        initTestCluster();
        final RestHelper rh = restHelper();
        rh.enableHTTPClientSSL = true;
        ...
        for (int i = 0; i < 100; i++) {
            RestHelper.HttpResponse resp = rh.executeGetRequest("_cat/nodes?h=ip,name,port,http_address,v,m&v");
    ...
    public void testCatHealth() throws Exception {
        initTestCluster();
        final EasyRestHelper rh = new EasyRestHelper(clusterInfo, "admin", "admin");

        for (int i = 0; i < 100; i++) {
            EasyRestHelper.HttpResponse resp = rh.executeGetRequest("_cat/nodes?h=ip,name,port,http_address,v,m&v", encodeBasicHeader("admin", "admin"));
            System.out.println(resp.getBody());
            resp = rh.executeGetRequest("_cat/health?v");
            System.out.println(resp.getBody());
            Thread.sleep(1000*5);
        }
    }
