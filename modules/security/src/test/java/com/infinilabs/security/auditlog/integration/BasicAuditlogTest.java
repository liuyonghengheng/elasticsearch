...
public class BasicAuditlogTest extends AbstractAuditlogiUnitTest {
    ...
    //needs proper ssl plugin version
        ...
        Settings additionalSettings = Settings.builder()
                ...
                .put("security.ssl.http.keystore_filepath", FileHelper.getAbsoluteFilePathFromClassPath("auditlog/node-0-keystore.jks"))
                .put("security.ssl.http.truststore_filepath", FileHelper.getAbsoluteFilePathFromClassPath("auditlog/truststore.jks"))
                .put("security.audit.type", TestAuditlogImpl.class.getName())
    ...
    public void testGrantedPrivilegesRest() throws Exception {
        ...
        testPrivilegeRest(HttpStatus.SC_OK, "/_security/role", AuditCategory.GRANTED_PRIVILEGES);
    }
    ...
    public void testMissingPrivilegesRest() throws Exception {
        ...
        testPrivilegeRest(HttpStatus.SC_FORBIDDEN, "/_security/role", AuditCategory.MISSING_PRIVILEGES);
    }
    ...
    public void testSecurityIndexAttempt() throws Exception {
        ...
        HttpResponse response = rh.executePutRequest(".security/config/0", "{}", encodeBasicHeader("admin", "admin"));
    ...
    public void testBadHeader() throws Exception {
        ...
        HttpResponse response = rh.executeGetRequest("", new BasicHeader("_security_bad", "bad"), encodeBasicHeader("admin", "admin"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, response.getStatusCode());
        Assert.assertFalse(TestAuditlogImpl.sb.toString(), TestAuditlogImpl.sb.toString().contains("AUTHENTICATED"));
        Assert.assertTrue(TestAuditlogImpl.sb.toString(), TestAuditlogImpl.sb.toString().contains("BAD_HEADERS"));
        Assert.assertTrue(TestAuditlogImpl.sb.toString(), TestAuditlogImpl.sb.toString().contains("_security_bad"));
    ...
    public void testIndexPattern() throws Exception {
        ...
        Settings additionalSettings = Settings.builder()
                .put("security.audit.type", "internal_elasticsearch")
                .put(ConfigConstants.SECURITY_AUDIT_LOG_REQUEST_BODY, false)
                .put(ConfigConstants.SECURITY_AUDIT_RESOLVE_INDICES, false)
                .put(ConfigConstants.SECURITY_AUDIT_CONFIG_DISABLED_TRANSPORT_CATEGORIES, "NONE")
                .put(ConfigConstants.SECURITY_AUDIT_CONFIG_DISABLED_REST_CATEGORIES, "NONE")
                .put("security.audit.threadpool.size", 10) //must be greater 0
                .put("security.audit.config.index", "'auditlog-'YYYY.MM.dd.ss")
                .build();
    ...
    public void testAliasBadHeaders() throws Exception {
        ...
        TestAuditlogImpl.clear();
        HttpResponse response = rh.executeGetRequest("_search?pretty", new BasicHeader("_security_user", "xxx"), encodeBasicHeader("admin", "admin"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, response.getStatusCode());
        System.out.println(TestAuditlogImpl.sb.toString());
    ...
    public void testRestMethod() throws Exception {
        ...
        // test PATCH
        TestAuditlogImpl.clear();
        rh.executePatchRequest("/_security/audit", "[]");
        Assert.assertEquals(1, TestAuditlogImpl.messages.size());
        ...
        TestAuditlogImpl.clear();
        rh.executePatchRequest("/_security/audit", "[]", adminHeader);
        Assert.assertEquals(2, TestAuditlogImpl.messages.size());
        Assert.assertTrue(TestAuditlogImpl.sb.toString().contains("MISSING_PRIVILEGES"));
        Assert.assertTrue(TestAuditlogImpl.sb.toString().contains("AUTHENTICATED"));
        Assert.assertTrue(TestAuditlogImpl.sb.toString().contains("\"audit_rest_request_method\" : \"PATCH\""));
        ...
        // test BAD_HEADERS
        TestAuditlogImpl.clear();
        rh.executeGetRequest("test", new BasicHeader("_security_user", "xxx"));
        Assert.assertEquals(1, TestAuditlogImpl.messages.size());
        Assert.assertTrue(TestAuditlogImpl.sb.toString().contains("BAD_HEADERS"));
        Assert.assertTrue(TestAuditlogImpl.sb.toString().contains("\"audit_rest_request_method\" : \"GET\""));
    }
    ...
    public void testSensitiveMethodRedaction() throws Exception {
        ...
        TestAuditlogImpl.clear();
        rh.executePutRequest("/_security/account", "{\"password\":\"new-pass\", \"current_password\":\"curr-passs\"}");
        Assert.assertEquals(1, TestAuditlogImpl.messages.size());
        Assert.assertTrue(TestAuditlogImpl.sb.toString().contains(expectedRequestBody));

        // test PUT internal users API
        TestAuditlogImpl.clear();
        rh.executePutRequest("/_security/user/test1", "{\"password\":\"new-pass\", \"external_roles\":[], \"attributes\": {}}");
        Assert.assertEquals(1, TestAuditlogImpl.messages.size());
        Assert.assertTrue(TestAuditlogImpl.sb.toString().contains(expectedRequestBody));

        // test PATCH internal users API
        TestAuditlogImpl.clear();
        rh.executePatchRequest("/_security/user/test1", "[{\"op\":\"add\", \"path\":\"/password\", \"value\": \"test-pass\"}]");
        Assert.assertEquals(1, TestAuditlogImpl.messages.size());
        Assert.assertTrue(TestAuditlogImpl.sb.toString().contains(expectedRequestBody));

        // test PUT users API
        TestAuditlogImpl.clear();
        rh.executePutRequest("/_security/user/test2", "{\"password\":\"new-pass\", \"external_roles\":[], \"attributes\": {}}");
        Assert.assertEquals(1, TestAuditlogImpl.messages.size());
        Assert.assertTrue(TestAuditlogImpl.sb.toString().contains(expectedRequestBody));
    }
}
