...
public class SnapshotRestoreTests extends SingleClusterTest {
    ...
    public void testSnapshotEnableSecurityIndexRestore() throws Exception {
        ...
        final Settings settings = Settings.builder()
                ...
                .put("security.check_snapshot_restore_write_privileges", false)
                .put("security.unsupported.restore.securityindex.enabled", true)
                .build();
        ...
        try (TransportClient tc = getInternalTransportClient()) {
            ...
            tc.admin().cluster().putRepository(new PutRepositoryRequest(".security").type("fs").settings(Settings.builder().put("location", repositoryPath.getRoot().getAbsolutePath() + "/.security"))).actionGet();
            tc.admin().cluster().createSnapshot(new CreateSnapshotRequest(".security", "security_1").indices(".security").includeGlobalState(false).waitForCompletion(true)).actionGet();
        ...
        // Try to restore vulcangov index as .security index, not possible since Security index is open
        Assert.assertEquals(HttpStatus.SC_INTERNAL_SERVER_ERROR, rh.executePostRequest("_snapshot/vulcangov/vulcangov_1/_restore?wait_for_completion=true","{ \"indices\": \"vulcangov\", \"rename_pattern\": \"(.+)\", \"rename_replacement\": \".security\" }", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());

        // Try to restore .security index.
        Assert.assertEquals(HttpStatus.SC_OK, rh.executeGetRequest("_snapshot/.security", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
        Assert.assertEquals(HttpStatus.SC_OK, rh.executeGetRequest("_snapshot/.security/security_1", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
        // 500 because Security index is open
        Assert.assertEquals(HttpStatus.SC_INTERNAL_SERVER_ERROR, rh.executePostRequest("_snapshot/.security/security_1/_restore?wait_for_completion=true","", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
        // Try to restore .security index as .security_copy index
        Assert.assertEquals(HttpStatus.SC_OK, rh.executePostRequest("_snapshot/.security/security_1/_restore?wait_for_completion=true","{ \"indices\": \".security\", \"rename_pattern\": \"(.+)\", \"rename_replacement\": \"security_copy\" }", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
        ...
        // Try to restore vulcangov index as .security index -> 500 because Security index is open
        Assert.assertEquals(HttpStatus.SC_INTERNAL_SERVER_ERROR, rh.executePostRequest("_snapshot/all/all_1/_restore?wait_for_completion=true","{ \"indices\": \"vulcangov\", \"rename_pattern\": \"(.+)\", \"rename_replacement\": \".security\" }", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
        // Try to restore .security index as .security_copy index. Delete security_copy first, was created in test above
        Assert.assertEquals(HttpStatus.SC_OK, rh.executeDeleteRequest("security_copy", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
        Assert.assertEquals(HttpStatus.SC_OK, rh.executePostRequest("_snapshot/all/all_1/_restore?wait_for_completion=true","{ \"indices\": \".security\", \"rename_pattern\": \"(.+)\", \"rename_replacement\": \"security_copy\" }", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
        ...
        Assert.assertEquals(HttpStatus.SC_OK, rh.executePostRequest(".security/_close", "", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
        Assert.assertEquals(HttpStatus.SC_OK, rh.executePostRequest("_snapshot/.security/security_1/_restore?wait_for_completion=true","", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
        Assert.assertEquals(HttpStatus.SC_OK, rh.executePostRequest(".security/_open", "", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
    }
    ...
    public void testSnapshot() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executePostRequest("_snapshot/vulcangov/vulcangov_1/_restore?wait_for_completion=true","", encodeBasicHeader("worf", "worf")).getStatusCode());
        // Try to restore vulcangov index as .security index
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executePostRequest("_snapshot/vulcangov/vulcangov_1/_restore?wait_for_completion=true","{ \"indices\": \"vulcangov\", \"rename_pattern\": \"(.+)\", \"rename_replacement\": \".security\" }", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());

        // Try to restore .security index.
        Assert.assertEquals(HttpStatus.SC_OK, rh.executeGetRequest("_snapshot/.security", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
        Assert.assertEquals(HttpStatus.SC_OK, rh.executeGetRequest("_snapshot/.security/security_1", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executePostRequest("_snapshot/.security/security_1/_restore?wait_for_completion=true","", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
        // Try to restore .security index as .security_copy index
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executePostRequest("_snapshot/.security/security_1/_restore?wait_for_completion=true","{ \"indices\": \".security\", \"rename_pattern\": \"(.+)\", \"rename_replacement\": \"security_copy\" }", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
        ...
        // Try to restore .security index as .security_copy index
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executePostRequest("_snapshot/all/all_1/_restore?wait_for_completion=true","{ \"indices\": \"vulcangov\", \"rename_pattern\": \"(.+)\", \"rename_replacement\": \".security\" }", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
        // Try to restore .security index as .security_copy index
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, rh.executePostRequest("_snapshot/all/all_1/_restore?wait_for_completion=true","{ \"indices\": \".security\", \"rename_pattern\": \"(.+)\", \"rename_replacement\": \"security_copy\" }", encodeBasicHeader("nagilum", "nagilum")).getStatusCode());
