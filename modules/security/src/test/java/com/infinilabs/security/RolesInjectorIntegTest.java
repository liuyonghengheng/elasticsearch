...
public class RolesInjectorIntegTest extends SingleClusterTest {
    ...
    private void waitForInit(Client client) throws Exception {
        try {
            ...
            if(ex.getMessage().contains("initializing")) {
                Thread.sleep(500);
                waitForInit(client);
            }
        }
    ...
    public void testRolesInject() throws Exception {
        setup(Settings.EMPTY, new DynamicSecurityConfig().setSecurityRoles("role.yml"), Settings.EMPTY);
        ...
        final Settings tcSettings = Settings.builder()
                ...
                .put("security.allow_default_init_securityindex", "true")
        ...
        RolesInjectorPlugin.injectedRoles = "valid_user|security_superuser";
        try (Node node = new PluginAwareNode(false, tcSettings, Netty4Plugin.class, SecurityPlugin.class, RolesInjectorPlugin.class).start()) {
            waitForInit(node.client());
