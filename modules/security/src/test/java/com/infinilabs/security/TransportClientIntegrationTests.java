...
public class TransportClientIntegrationTests extends SingleClusterTest {
	...
	public void testTransportClient() throws Exception {
		...
		Settings tcSettings = Settings.builder()
				.put(settings)
				.put("security.ssl.transport.keystore_filepath", FileHelper.getAbsoluteFilePathFromClassPath("spock-keystore.jks"))
		...
		try (TransportClient tc = getInternalTransportClient(clusterInfo, tcSettings)) {
			...
			gr =tc.prepareGet(".security", "security", "config").setRealtime(false).get();
			Assert.assertFalse(gr.isExists());

			System.out.println("------- 7 ---------");

			gr =tc.prepareGet(".security", "security", "config").setRealtime(true).get();
			Assert.assertFalse(gr.isExists());

			System.out.println("------- 8 ---------");

			actionGet = tc.search(new SearchRequest(".security")).actionGet();
			Assert.assertEquals(0, actionGet.getHits().getHits().length);

			System.out.println("------- 9 ---------");

			try {
				tc.index(new IndexRequest(".security").type(getType()).id("config").source("config", FileHelper.readYamlContent("config.yml"))).actionGet();
				Assert.fail();
			} catch (Exception e) {
			...
			try {
				...
				try {
					tc.threadPool().getThreadContext().putHeader("security_run_as", "worf");
			...
			try {
				ctx = tc.threadPool().getThreadContext().stashContext();
				try {
					tc.threadPool().getThreadContext().putHeader("security_run_as", "gkar");
					gr = tc.prepareGet("vulcan", "secrets", "s1").get();
					Assert.fail();
				} finally {
					ctx.close();
			...
			ctx = tc.threadPool().getThreadContext().stashContext();
			try {
				tc.threadPool().getThreadContext().putHeader("security_run_as", "nagilum");
				gr = tc.prepareGet(".security", "security", "config").setRealtime(Boolean.TRUE).get();
				Assert.assertFalse(gr.isExists());
			...
			try {
				tc.threadPool().getThreadContext().putHeader("security_run_as", "nagilum");
				gr = tc.prepareGet(".security", "config", "0").setRealtime(Boolean.FALSE).get();
				Assert.assertFalse(gr.isExists());
				Assert.assertTrue(gr.isSourceEmpty());
			} finally {
				ctx.close();
			...
			try {
				tc.threadPool().getThreadContext().putHeader("security_run_as", "nagilum");
				gr = tc.prepareGet(".security", "security", "config").setRealtime(Boolean.FALSE).get();
				Assert.assertFalse(gr.isExists());
				Assert.assertTrue(gr.isSourceEmpty());
			} finally {
				ctx.close();
	...
	public void testTransportClientImpersonation() throws Exception {
		...
		final Settings settings = Settings.builder()
				.putList("security.authcz.impersonation_dn.CN=spock,OU=client,O=client,L=Test,C=DE", "worf", "nagilum")
				.build();
		...
		Settings tcSettings = Settings.builder()
				...
				.put(SSLConfigConstants.SECURITY_SSL_TRANSPORT_KEYSTORE_ALIAS,"spock")
				.put("path.home", ".")
				.put("request.headers.security_run_as", "worf")
				.build();
	...
	public void testTransportClientImpersonationWildcard() throws Exception {
		...
		final Settings settings = Settings.builder()
				.putList("security.authcz.impersonation_dn.CN=spock,OU=client,O=client,L=Test,C=DE", "*")
				.build();
	...
	public void testTransportClientUsernameAttribute() throws Exception {
		...
		setup(Settings.EMPTY, new DynamicSecurityConfig().setConfig("config_transport_username.yml")
				.setSecurityRolesMapping("role_mapping_transport_username.yml")
