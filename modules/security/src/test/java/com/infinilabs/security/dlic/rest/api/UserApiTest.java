...
public class UserApiTest extends AbstractRestApiUnitTest {
    ...
    public void testSecurityRoles() throws Exception {
        ...
        response = rh.executePatchRequest("/_security/user", "[{ \"op\": \"add\", \"path\": \"/newuser\", \"value\": {\"password\": \"newuser\", \"roles\": [\"security_superuser\"] } }]", new Header[0]);
        Assert.assertEquals(response.getBody(), HttpStatus.SC_OK, response.getStatusCode());

        response = rh.executeGetRequest("/_security/user/newuser", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        Assert.assertTrue(response.getBody().contains("\"roles\":[\"security_superuser\"]"));
    ...
    public void testUserApi() throws Exception {
        ...
        response = rh.executeGetRequest("/_security/user/admin", new Header[0]);
        Assert.assertEquals(response.getBody(), HttpStatus.SC_OK, response.getStatusCode());
        ...
        response = rh.executeGetRequest("/_security/user/nothinghthere", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());

        // GET, new URL endpoint in security
        response = rh.executeGetRequest("/_security/user/", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        ...
        // no username given
        response = rh.executePutRequest("/_security/user/", "{\"hash\": \"123\"}", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_METHOD_NOT_ALLOWED, response.getStatusCode());

        // Faulty JSON payload
        response = rh.executePutRequest("/_security/user/nagilum", "{some: \"thing\" asd  other: \"thing\"}",
                new Header[0]);
        ...
        response = rh.executePutRequest("/_security/user/nagilum", "{some: \"thing\", other: \"thing\"}",
                new Header[0]);
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
        settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
        ...
        // Get hidden role
        response = rh.executeGetRequest("/_security/user/hide" , new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        Assert.assertTrue(response.getBody().contains("\"hidden\":true"));

        // Associating with hidden role is allowed (for superadmin)
        response = rh.executePutRequest("/_security/user/test", "{ \"roles\": " +
                "[\"security_hidden\"]}", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

        // Associating with reserved role is allowed (for superadmin)
        response = rh.executePutRequest("/_security/user/test", "{ \"roles\": [\"security_reserved\"], " +
                "\"hash\": \"123\"}",
            new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

        // Associating with non-existent role is not allowed
        response = rh.executePutRequest("/_security/user/nagilum", "{ \"roles\": [\"non_existent\"]}",
            new Header[0]);
        Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());
        settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
        ...
        rh.sendAdminCertificate = true;
        response = rh.executePatchRequest("/_security/user/imnothere", "[{ \"op\": \"add\", \"path\": \"/a/b/c\", \"value\": [ \"foo\", \"bar\" ] }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());

        // PATCH read only resource, must be forbidden,
        // but SuperAdmin can PATCH read-only resource
        rh.sendAdminCertificate = true;
        response = rh.executePatchRequest("/_security/user/sarek", "[{ \"op\": \"add\", \"path\": \"/description\", \"value\": \"foo\" }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

        // PATCH hidden resource, must be not found, can be found for super admin
        rh.sendAdminCertificate = true;
        response = rh.executePatchRequest("/_security/user/q", "[{ \"op\": \"add\", \"path\": \"/a/b/c\", \"value\": [ \"foo\", \"bar\" ] }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());

        // PATCH value of hidden flag, must fail with validation error
        rh.sendAdminCertificate = true;
        response = rh.executePatchRequest("/_security/user/test", "[{ \"op\": \"add\", \"path\": \"/hidden\", \"value\": true }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
        Assert.assertTrue(response.getBody().matches(".*\"invalid_keys\"\\s*:\\s*\\{\\s*\"keys\"\\s*:\\s*\"hidden\"\\s*\\}.*"));

        // PATCH password
        rh.sendAdminCertificate = true;
        response = rh.executePatchRequest("/_security/user/test", "[{ \"op\": \"add\", \"path\": \"/password\", \"value\": \"neu\" }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        response = rh.executeGetRequest("/_security/user/test", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
        ...
        // PATCH on non-existing resource
        rh.sendAdminCertificate = true;
        response = rh.executePatchRequest("/_security/user", "[{ \"op\": \"add\", \"path\": \"/imnothere/a\", \"value\": [ \"foo\", \"bar\" ] }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());

        // PATCH read only resource, must be forbidden,
        // but SuperAdmin can PATCH read only resouce
        rh.sendAdminCertificate = true;
        response = rh.executePatchRequest("/_security/user", "[{ \"op\": \"add\", \"path\": \"/sarek/description\", \"value\": \"foo\" }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

        rh.sendAdminCertificate = false;
        response = rh.executePatchRequest("/_security/user", "[{ \"op\": \"add\", \"path\": \"/sarek/a\", \"value\": [ \"foo\", \"bar\" ] }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_UNAUTHORIZED, response.getStatusCode());

        // PATCH hidden resource, must be bad request
        rh.sendAdminCertificate = true;
        response = rh.executePatchRequest("/_security/user", "[{ \"op\": \"add\", \"path\": \"/q/a\", \"value\": [ \"foo\", \"bar\" ] }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());

        // PATCH value of hidden flag, must fail with validation error
        rh.sendAdminCertificate = true;
        response = rh.executePatchRequest("/_security/user", "[{ \"op\": \"add\", \"path\": \"/test/hidden\", \"value\": true }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
        Assert.assertTrue(response.getBody().matches(".*\"invalid_keys\"\\s*:\\s*\\{\\s*\"keys\"\\s*:\\s*\"hidden\"\\s*\\}.*"));

        // PATCH
        rh.sendAdminCertificate = true;
        response = rh.executePatchRequest("/_security/user", "[{ \"op\": \"add\", \"path\": \"/bulknew1\", \"value\": {\"password\": \"bla\", \"external_roles\": [\"vulcan\"] } }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        response = rh.executeGetRequest("/_security/user/bulknew1", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
        Assert.assertFalse(settings.hasValue("bulknew1.password"));
        Assert.assertTrue(settings.hasValue("bulknew1.hash"));
        List<String> roles = settings.getAsList("bulknew1.external_roles");
        Assert.assertEquals(1, roles.size());
        Assert.assertTrue(roles.contains("vulcan"));

        // add user with correct setting. User is in role "security_superuser"
        ...
        rh.sendAdminCertificate = true;
        response = rh.executeDeleteRequest("/_security/user", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_METHOD_NOT_ALLOWED, response.getStatusCode());

        // try remove user, nonexisting user
        response = rh.executeDeleteRequest("/_security/user/picard", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());

        // try remove readonly user
        response = rh.executeDeleteRequest("/_security/user/sarek", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

        // try remove hidden user, allowed for super admin
        response = rh.executeDeleteRequest("/_security/user/q", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        ...
        response = rh.executeGetRequest("/_security/user/nagilum", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
        ...
        rh.sendAdminCertificate = true;
        response = rh.executePutRequest("/_security/user/picard", FileHelper.loadFile("restapi/users_wrong_datatypes.json"), new Header[0]);
        settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
        ...
        rh.sendAdminCertificate = true;
        response = rh.executePutRequest("/_security/user/picard", FileHelper.loadFile("restapi/users_wrong_datatypes2.json"), new Header[0]);
        settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
        Assert.assertEquals(AbstractConfigurationValidator.ErrorType.WRONG_DATATYPE.getMessage(), settings.get("reason"));
        ...
        rh.sendAdminCertificate = true;
        response = rh.executePutRequest("/_security/user/picard", FileHelper.loadFile("restapi/users_wrong_datatypes3.json"), new Header[0]);
        settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
        Assert.assertEquals(AbstractConfigurationValidator.ErrorType.WRONG_DATATYPE.getMessage(), settings.get("reason"));
        Assert.assertTrue(settings.get("external_roles").equals("Array expected"));
        ...
        rh.sendAdminCertificate = true;
        response = rh.executeGetRequest("/_security/user/picard", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
        Assert.assertEquals("", settings.get("picard.hash"));
        roles = settings.getAsList("picard.external_roles");
        ...
        // check tabs in json
        response = rh.executePutRequest("/_security/user/userwithtabs", "\t{\"hash\": \t \"123\"\t}  ", new Header[0]);
    ...
    public void testPasswordRules() throws Exception {
        ...
        response = rh.executePatchRequest("/_security/user", "[{ \"op\": \"add\", \"path\": \"/ok4\", \"value\": {\"password\": \"bla\", \"external_roles\": [\"vulcan\"] } }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());

        response = rh.executePatchRequest("/_security/user", "[{ \"op\": \"replace\", \"path\": \"/ok4\", \"value\": {\"password\": \"bla\", \"external_roles\": [\"vulcan\"] } }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());

        addUserWithPassword("ok4", "123", HttpStatus.SC_BAD_REQUEST);

        response = rh.executePatchRequest("/_security/user", "[{ \"op\": \"add\", \"path\": \"/ok4\", \"value\": {\"password\": \"$1aAAAAAAAAB\", \"external_roles\": [\"vulcan\"] } }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

        addUserWithPassword("ok4", "$1aAAAAAAAAC", HttpStatus.SC_OK);

        //its not allowed to use the username as password (case insensitive)
        response = rh.executePatchRequest("/_security/user", "[{ \"op\": \"add\", \"path\": \"/$1aAAAAAAAAB\", \"value\": {\"password\": \"$1aAAAAAAAAB\", \"external_roles\": [\"vulcan\"] } }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
        ...
        response = rh.executeGetRequest("/_security/user/nothinghthere?pretty", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());
        Assert.assertTrue(response.getBody().contains("NOT_FOUND"));

        String patchPayload = "[ " +
                "{ \"op\": \"add\", \"path\": \"/testuser1\",  \"value\": { \"password\": \"$aA123456789\", \"external_roles\": [\"testrole1\"] } }," +
                "{ \"op\": \"add\", \"path\": \"/testuser2\",  \"value\": { \"password\": \"testpassword2\", \"external_roles\": [\"testrole2\"] } }" +
                "]";

        response = rh.executePatchRequest("/_security/user", patchPayload, new BasicHeader("Content-Type", "application/json"));
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
        Assert.assertTrue(response.getBody().contains("error"));
        Assert.assertTrue(response.getBody().contains("xxx"));

        response = rh.executePutRequest("/_security/user/ok1", "{\"external_roles\":[\"my-backend-role\"],\"attributes\":{},\"password\":\"\"}", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());

        response = rh.executePutRequest("/_security/user/ok1", "{\"external_roles\":[\"my-backend-role\"],\"attributes\":{},\"password\":\"Admin_123\"}", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

        response = rh.executePutRequest("/_security/user/ok1", "{\"external_roles\":[\"my-backend-role\"],\"attributes\":{}}", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

        response = rh.executePutRequest("/_security/user/ok1", "{\"external_roles\":[\"my-backend-role\"],\"attributes\":{},\"password\":\"bla\"}",
                new Header[0]);
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
    }
    ...
    public void testUserApiNoPasswordChange() throws Exception {
        ...
        response = rh.executePutRequest("/_security/user/user1", "{\"hash\":\"$2a$12$n5nubfWATfQjSYHiWtUyeOxMIxFInUHOAx8VMmGmxFNPGpaBmeB.m\",\"password\":\"\",\"external_roles\":[\"admin\",\"rolea\"]}");
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());

        response = rh.executePutRequest("/_security/user/user1", "{\"hash\":\"$2a$12$n5nubfWATfQjSYHiWtUyeOxMIxFInUHOAx8VMmGmxFNPGpaBmeB.m\",\"password\":\"Admin_123\",\"external_roles\":[\"admin\",\"rolea\"]}");
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

        response = rh.executeGetRequest("/_security/user/user1");
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

        addUserWithHash("user2", "$2a$12$n5nubfWATfQjSYHiWtUyeOxMIxFInUHOAx8VMmGmxFNPGpaBmeB.m",
                HttpStatus.SC_CREATED);

        response = rh.executePutRequest("/_security/user/user2", "{\"password\":\"\",\"external_roles\":[\"admin\",\"rolex\"]}");
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());

        response = rh.executePutRequest("/_security/user/user2", "{\"password\":\"Admin_123\",\"external_roles\":[\"admin\",\"rolex\"]}");
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

        response = rh.executeGetRequest("/_security/user/user2");
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
    }
    ...
    public void testUserApiForNonSuperAdmin() throws Exception {
        ...
        response = rh.executePatchRequest("/_security/user/sarek", "[{ \"op\": \"add\", \"path\": \"/sarek/description\", \"value\": \"foo\" }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, response.getStatusCode());

        // Put read only users
        response = rh.executePutRequest("/_security/user/sarek", "{ \"roles\": [\"security_reserved\"]}", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, response.getStatusCode());
        ...
        // Delete hidden user
        response = rh.executeDeleteRequest("/_security/user/hide" , new Header[0]);
        Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());

        // Patch hidden users
        response = rh.executePatchRequest("/_security/user/hide", "[{ \"op\": \"add\", \"path\": \"/sarek/description\", \"value\": \"foo\" }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());

        // Put hidden users
        response = rh.executePutRequest("/_security/user/hide", "{ \"roles\": [\"security_reserved\"]}", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());

        // Put reserved role is forbidden for non-superadmin
        response = rh.executePutRequest("/_security/user/nagilum", "{ \"roles\": [\"security_reserved\"]}",
            new Header[0]);
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, response.getStatusCode());
        Settings settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
        Assert.assertEquals(settings.get("message"), "Resource 'security_reserved' is read-only.");

        // Patch single hidden user
        response = rh.executePatchRequest("/_security/user/hide", "[{ \"op\": \"add\", \"path\": \"/description\", \"value\": \"foo\" }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());

        // Patch multiple hidden users
        response = rh.executePatchRequest("/_security/user", "[{ \"op\": \"add\", \"path\": \"/hide/description\", \"value\": \"foo\" }]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());
