...
import com.infinilabs.security.securityconf.SecurityRoles;
...
import static com.infinilabs.security.privileges.PrivilegesInterceptor.CONTINUE_EVALUATION_REPLACE_RESULT;
...
public class PrivilegesEvaluator {
    ...
    private void setUserInfoInThreadContext(User user, Set<String> mappedRoles) {
        if (threadContext.getTransient(ConfigConstants.SECURITY_USER_INFO_THREAD_CONTEXT) == null) {
            StringJoiner joiner = new StringJoiner("|");
            joiner.add(user.getName());
            joiner.add(String.join(",", user.getRoles()));
            joiner.add(String.join(",", Sets.union(user.getSecurityRoles(), mappedRoles)));
            threadContext.putTransient(ConfigConstants.SECURITY_USER_INFO_THREAD_CONTEXT, joiner.toString());
        }
    }
    ...
    public PrivilegesEvaluatorResponse evaluate(final User user, String action0, final ActionRequest request,
        ...
        if (!isInitialized()) {
            throw new ElasticsearchSecurityException("Security is not initialized.");
        }
        ...
        final String injectedRolesValidationString = threadContext.getTransient(ConfigConstants.SECURITY_INJECTED_ROLES_VALIDATION);
            ...
            if(!securityRoles.impliesClusterPermissionPermission(action0)) {
                ...
                if(request instanceof RestoreSnapshotRequest && checkSnapshotRestoreWritePrivileges) {
                    ...
                    if(privilegesInterceptor.getClass() != PrivilegesInterceptor.class) {
                        ...
                        final PrivilegesInterceptor.ReplaceResult replaceResult =CONTINUE_EVALUATION_REPLACE_RESULT;// privilegesInterceptor.replaceKibanaIndex(request, action0, user, dcm, requestedResolved,
//                                mapTenants(user, mappedRoles));
        ...
        if(privilegesInterceptor.getClass() != PrivilegesInterceptor.class) {
            ...
            final PrivilegesInterceptor.ReplaceResult replaceResult = CONTINUE_EVALUATION_REPLACE_RESULT;
    ...
    private boolean checkFilteredAliases(Set<String> requestedResolvedIndices, String action) {
        ...
        for (String requestAliasOrIndex: requestedResolvedIndices) {
            ...
            if(filteredAliases.size() > 1 && ACTION_MATCHER.test(action)) {
                //TODO add queries as query queries (works only if query module is installed)
