...
import com.infinilabs.security.auditlog.config.ThreadPoolConfig;
...
import com.infinilabs.security.auditlog.sink.SinkProvider;
...
public class AuditMessageRouter {
    ...
    public final void route(final AuditMessage msg) {
        if (!isEnabled()) {
            // should not happen since we check in AuditLogging, so this is just a safeguard
