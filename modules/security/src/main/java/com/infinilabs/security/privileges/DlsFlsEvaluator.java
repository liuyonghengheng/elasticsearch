...
public class DlsFlsEvaluator {
    ...
    public PrivilegesEvaluatorResponse evaluate(final ActionRequest request, final ClusterService clusterService, final IndexNameExpressionResolver resolver, final Resolved requestedResolved, final User user,
            ...
            if(request instanceof ClusterSearchShardsRequest && HeaderHelper.isTrustedClusterRequest(threadContext)) {
                ...
                if (threadContext.getHeader(ConfigConstants.SECURITY_MASKED_FIELD_HEADER) != null) {
                    if (!maskedFieldsMap.equals(Base64Helper.deserializeObject(threadContext.getHeader(ConfigConstants.SECURITY_MASKED_FIELD_HEADER)))) {
                        throw new ElasticsearchSecurityException(ConfigConstants.SECURITY_MASKED_FIELD_HEADER + " does not match  ");
                    } else {
                        if (log.isDebugEnabled()) {
                            log.debug(ConfigConstants.SECURITY_MASKED_FIELD_HEADER + " already set");
                        }
                    }
                } else {
        ...
        // attach query/field_security map if not already done
            ...
            if(request instanceof ClusterSearchShardsRequest && HeaderHelper.isTrustedClusterRequest(threadContext)) {
                ...
                if (threadContext.getHeader(ConfigConstants.SECURITY_DLS_QUERY_HEADER) != null) {
                    if (!dlsQueries.equals(Base64Helper.deserializeObject(threadContext.getHeader(ConfigConstants.SECURITY_DLS_QUERY_HEADER)))) {
                        throw new ElasticsearchSecurityException(ConfigConstants.SECURITY_DLS_QUERY_HEADER + " does not match (SG 900D)");
                    }
                } else {
            ...
            if(request instanceof ClusterSearchShardsRequest && HeaderHelper.isTrustedClusterRequest(threadContext)) {
                ...
                if (threadContext.getHeader(ConfigConstants.SECURITY_FLS_FIELDS_HEADER) != null) {
                    if (!flsFields.equals(Base64Helper.deserializeObject(threadContext.getHeader(ConfigConstants.SECURITY_FLS_FIELDS_HEADER)))) {
                        throw new ElasticsearchSecurityException(ConfigConstants.SECURITY_FLS_FIELDS_HEADER + " does not match  ");
                    } else {
                        if (log.isDebugEnabled()) {
                            log.debug(ConfigConstants.SECURITY_FLS_FIELDS_HEADER + " already set");
                        }
                    }
                } else {
