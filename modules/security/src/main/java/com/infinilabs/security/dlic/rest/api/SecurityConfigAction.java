...
import com.infinilabs.security.dlic.rest.validation.SecurityConfigValidator;
...
public class SecurityConfigAction extends PatchableResourceApiAction {
    ...
    private static final List<Route> getRoutes = Collections.singletonList(
            new Route(Method.GET, "/_security/securityconfig/")
    );

    private static final List<Route> allRoutes = new ImmutableList.Builder<Route>()
            .addAll(getRoutes)
            .add(new Route(Method.PUT, "/_security/securityconfig/{name}"))
            .add(new Route(Method.PATCH, "/_security/securityconfig/"))
            .build();
