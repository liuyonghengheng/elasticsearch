...
package com.infinilabs.security.securityconf;
...
import com.infinilabs.security.securityconf.impl.WhitelistingSettings;
import com.infinilabs.security.support.WildcardMatcher;
import com.google.common.collect.ImmutableList;
import com.infinilabs.security.configuration.ClusterInfoHolder;
import com.infinilabs.security.configuration.ConfigurationChangeListener;
import com.infinilabs.security.configuration.StaticResourceException;
...
import com.infinilabs.security.auth.internal.InternalAuthenticationBackend;
...
import com.infinilabs.security.securityconf.impl.v6.ConfigV6;
...
public class DynamicConfigFactory implements Initializable, ConfigurationChangeListener {
    ...
    private void loadStaticConfig() throws IOException {
        JsonNode staticRolesJsonNode = DefaultObjectMapper.YAML_MAPPER
                .readTree(DynamicConfigFactory.class.getResourceAsStream("/static_config/static_role.yml"));
        staticRoles = SecurityDynamicConfiguration.fromNode(staticRolesJsonNode, CType.ROLE, 2, 0, 0);

        JsonNode staticActionGroupsJsonNode = DefaultObjectMapper.YAML_MAPPER
                .readTree(DynamicConfigFactory.class.getResourceAsStream("/static_config/static_privilege.yml"));
        staticActionGroups = SecurityDynamicConfiguration.fromNode(staticActionGroupsJsonNode, CType.PRIVILEGE, 2, 0, 0);
    }

    public final static SecurityDynamicConfiguration<?> addStatics(SecurityDynamicConfiguration<?> original) {
        if(original.getCType() == CType.PRIVILEGE && !staticActionGroups.getCEntries().isEmpty()) {
            original.add(staticActionGroups.deepClone());
        }

        if(original.getCType() == CType.ROLE && !staticRoles.getCEntries().isEmpty()) {
    ...
    public DynamicConfigFactory(ConfigurationRepository cr, final Settings esSettings,
        ...
        if(esSettings.getAsBoolean(ConfigConstants.SECURITY_UNSUPPORTED_LOAD_STATIC_RESOURCES, true)) {
            try {
                loadStaticConfig();
            } catch (IOException e) {
    ...
    public void onChange(Map<CType, SecurityDynamicConfiguration<?>> typeToConfig) {
        ...
        SecurityDynamicConfiguration<?> actionGroups = cr.getConfiguration(CType.PRIVILEGE);
        config = cr.getConfiguration(CType.CONFIG);
        SecurityDynamicConfiguration<?> user = cr.getConfiguration(CType.USER);
        SecurityDynamicConfiguration<?> roles = cr.getConfiguration(CType.ROLE);
        SecurityDynamicConfiguration<?> role_mapping = cr.getConfiguration(CType.ROLE_MAPPING);
        ...
        if (log.isDebugEnabled()) {
            String logmsg = "current config (because of " + typeToConfig.keySet() + ")\n" +
                    ...
                    " user: " + user.getImplementingClass() + " with " + user.getCEntries().size() + " entries\n" +
                    " roles: " + roles.getImplementingClass() + " with " + roles.getCEntries().size() + " entries\n" +
                    " role_mapping: " + role_mapping.getImplementingClass() + " with " + role_mapping.getCEntries().size() + " entries\n" +
        ...
        if(config.getImplementingClass() == ConfigV7.class) {
            ...
            ium = new InternalUsersModelV7((SecurityDynamicConfiguration<InternalUserV7>) user,
                (SecurityDynamicConfiguration<RoleV7>) roles,
                (SecurityDynamicConfiguration<RoleMappingsV7>) role_mapping);
            cm = new ConfigModelV7((SecurityDynamicConfiguration<RoleV7>) roles,(SecurityDynamicConfiguration<RoleMappingsV7>)role_mapping, (SecurityDynamicConfiguration<ActionGroupsV7>)actionGroups, dcm, esSettings);

        } else {

            //rebuild v6 Models
            dcm = new DynamicConfigModelV6(getConfigV6(config), esSettings, configPath, iab);
            ium = new InternalUsersModelV6((SecurityDynamicConfiguration<InternalUserV6>) user);
            cm = new ConfigModelV6((SecurityDynamicConfiguration<RoleV6>) roles, (SecurityDynamicConfiguration<ActionGroupsV6>)actionGroups, (SecurityDynamicConfiguration<RoleMappingsV6>)role_mapping, dcm, esSettings);
    ...
    private static class InternalUsersModelV7 extends InternalUsersModel {
        ...
        public List<String> getBackenRoles(String user) {
            ...
            return tmp==null?null:tmp.getExternal_roles();
        }
        ...
        public List<String> getSecurityRoles(String user) {
            ...
            // We should filter out any roles that have hidden role_mapping.
            return tmp == null ? ImmutableList.of() :
                tmp.getRoles().stream().filter(role -> !isRolesMappingHidden(role) && rolesV7SecurityDynamicConfiguration.exists(role)).collect(ImmutableList.toImmutableList());
        }

        // Remove any hidden role_mapping from the opendistro security roles
