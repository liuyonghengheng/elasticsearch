...
package com.infinilabs.security.auth.internal;
...
import com.infinilabs.security.securityconf.InternalUsersModel;
...
public class InternalAuthenticationBackend implements AuthenticationBackend, AuthorizationBackend {
    ...
    public boolean exists(User user) {
        ...
        if(exists) {
            ...
            final List<String> openDistroSecurityRoles = internalUsersModel.getSecurityRoles(user.getName());
            if(openDistroSecurityRoles != null) {
                user.addSecurityRoles(openDistroSecurityRoles);
            }
    ...
    public User authenticate(final AuthCredentials credentials) {
        ...
        if (internalUsersModel == null) {
            throw new ElasticsearchSecurityException("Internal authentication backend not configured. May be not initialized.");
        }
        ...
        try {
            if (OpenBSDBCrypt.checkPassword(internalUsersModel.getHash(credentials.getUsername()), array)) {
                ...
                final List<String> openDistroSecurityRoles = internalUsersModel.getSecurityRoles(credentials.getUsername());
                if(openDistroSecurityRoles != null) {
                    user.addSecurityRoles(openDistroSecurityRoles);
                }
