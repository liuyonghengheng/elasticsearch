...
public class ConfigModelV7 extends ConfigModel {
    ...
    public ConfigModelV7(
            ...
            SecurityDynamicConfiguration<ActionGroupsV7> privilege,
        ...
        try {
            rolesMappingResolution = ConfigConstants.RolesMappingResolution.valueOf(
                    esSettings.get(ConfigConstants.SECURITY_ROLES_MAPPING_RESOLUTION, ConfigConstants.RolesMappingResolution.MAPPING_ONLY.toString())
                            .toUpperCase());
        } catch (Exception e) {
        ...
        agr = reloadActionGroups(privilege);
        ...
        return new ActionGroupResolver() {
            ...
            private Set<String> resolve(final SecurityDynamicConfiguration<?> actionGroups, final String entry) {
                ...
                if(actionGroupAsObject != null && actionGroupAsObject instanceof List) {
                    ...
                    for (final String perm: ((ActionGroupsV7) actionGroupAsObject).getPrivileges()) {
                        if (actionGroups.getCEntries().keySet().contains(perm)) {
                            ret.addAll(resolve(actionGroups,perm));
                        } else {
                            ret.add(perm);
            ...
            Future<SecurityRole> future = execs.submit(new Callable<SecurityRole>() {
                ...
                public SecurityRole call() throws Exception {
                    ...
                    _securityRole.addClusterPerms(permittedClusterActions);
                        ...
                        for (final RoleV7.Index permittedAliasesIndex : securityRole.getValue().getIndices()) {

                            final String query = permittedAliasesIndex.getQuery();
                            final List<String> field_security = permittedAliasesIndex.getField_security();
                            final List<String> maskedFields = permittedAliasesIndex.getField_mask();

                            for(String pat: permittedAliasesIndex.getNames()) {
                                IndexPattern _indexPattern = new IndexPattern(pat);
                                _indexPattern.setDlsQuery(query);
                                _indexPattern.addFlsFields(field_security);
                                _indexPattern.addMaskedFields(maskedFields);
                                _indexPattern.addPerm(agr.resolvedActions(permittedAliasesIndex.getPrivileges()));
    ...
    public static class SecurityRoles implements com.infinilabs.security.securityconf.SecurityRoles {
        ...
        public Tuple<Map<String, Set<String>>, Map<String, Set<String>>> getDlsFls(User user, IndexNameExpressionResolver resolver,
            ...
            for (SecurityRole sr : roles) {
                for (IndexPattern ip : sr.getIpatterns()) {
                    final Set<String> field_security = ip.getFls();
                    final String query = ip.getDlsQuery(user);
                    final String indexPattern = ip.getUnresolvedIndexPattern(user);
                    Set<String> concreteIndices = new HashSet<>();

                    if ((query != null && query.length() > 0) || (field_security != null && field_security.size() > 0)) {
                    ...
                    if (query != null && query.length() > 0) {
                        ...
                        if (dlsQuery != null) {
                            ...
                            dlsQueries.put(indexPattern, new HashSet<>(Arrays.asList(query)));
                        }
                        ...
                        for (String concreteIndex : concreteIndices) {
                            ...
                            if (dlsQuery != null) {
                                ...
                                dlsQueries.put(concreteIndex, new HashSet<>(Arrays.asList(query)));
                            }
                        }

                    }

                    if (field_security != null && field_security.size() > 0) {

                        Set<String> flsField = flsFields.get(indexPattern);
                        if (flsField != null) {
                            flsField.addAll(field_security);
                        } else {
                            flsFields.put(indexPattern, new HashSet<>(field_security));
                        }
                        ...
                        for (String concreteIndex : concreteIndices) {
                            ...
                            if (flsField != null) {
                                ...
                                flsFields.put(concreteIndex, new HashSet<>(field_security));
                            }
                        }
                    }
                }
...
//                }
    ...
    public static class IndexPattern {
        ...
        private final Set<String> field_security = new HashSet<>();
        ...
        public IndexPattern addFlsFields(List<String> flsFields) {
            if (flsFields != null) {
                this.field_security.addAll(flsFields);
            }
            return this;
        }
        ...
        public int hashCode() {
            ...
            result = prime * result + ((field_security == null) ? 0 : field_security.hashCode());
        ...
        public boolean equals(Object obj) {
            ...
            if (field_security == null) {
                if (other.field_security != null)
                    return false;
            } else if (!field_security.equals(other.field_security))
                return false;
            if (maskedFields == null) {
        ...
        public String toString() {
            return System.lineSeparator() + "        indexPattern=" + indexPattern + System.lineSeparator() + "          dlsQuery=" + dlsQuery
                    + System.lineSeparator() + "          field_security=" + field_security + System.lineSeparator() + "          perms=" + perms;
        }
        ...
        public Set<String> getFls() {
            return Collections.unmodifiableSet(field_security);
        }
    ...
    private static String replaceProperties(String orig, User user) {
        ...
        orig = orig.replace("${user.name}", user.getName()).replace("${username}", user.getName());
        ...
        private RoleMappingHolder(final SecurityDynamicConfiguration<RoleMappingsV7> rolemappings, final String hostResolverMode) {
            ...
            if (roles != null) {
                ...
                for (final Entry<String, RoleMappingsV7> roleMap : rolemappings.getCEntries().entrySet()) {
                    ...
                    final Set<String> abar = new HashSet<>(roleMapValue.getAnd_external_roles());

                    if (!abar.isEmpty()) {
                        abars.put(WildcardMatcher.matchers(abar), roleMapKey);
                    }

                    for (String bar : roleMapValue.getExternal_roles()) {
