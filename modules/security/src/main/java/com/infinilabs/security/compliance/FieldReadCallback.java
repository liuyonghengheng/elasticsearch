...
package com.infinilabs.security.compliance;
...
import com.infinilabs.security.support.SourceFieldsContext;
...
public final class FieldReadCallback {
    ...
    public FieldReadCallback(final ThreadContext threadContext, final IndexService indexService,
        ...
        try {
            sfc = (SourceFieldsContext) HeaderHelper.deserializeSafeFromHeader(threadContext, "_security_source_field_context");
            if(sfc != null && sfc.hasIncludesOrExcludes()) {
                if(log.isTraceEnabled()) {
                    log.trace("_security_source_field_context: "+sfc);
                }
            ...
            if(log.isDebugEnabled()) {
                log.debug("Cannot deserialize _security_source_field_context because of {}", e.toString());
            }
        }
    }
