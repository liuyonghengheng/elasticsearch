...
import com.infinilabs.security.ssl.util.CertFileProps;
import com.infinilabs.security.ssl.util.CertFromFile;
import com.infinilabs.security.ssl.util.CertFromKeystore;
import com.infinilabs.security.ssl.util.CertFromTruststore;
import com.infinilabs.security.ssl.util.ExceptionUtils;
import com.infinilabs.security.ssl.util.KeystoreProps;
...
public class DefaultSecurityKeyStore implements SecurityKeyStore {
    ...
    public DefaultSecurityKeyStore(final Settings settings, final Path configPath) {
        ...
        final boolean useOpenSSLForHttpIfAvailable = SecuritySSLPlugin.OPENSSL_SUPPORTED && settings
                .getAsBoolean(SSLConfigConstants.SECURITY_SSL_HTTP_ENABLE_OPENSSL_IF_AVAILABLE, true);
        final boolean useOpenSSLForTransportIfAvailable = SecuritySSLPlugin.OPENSSL_SUPPORTED && settings
                .getAsBoolean(SSLConfigConstants.SECURITY_SSL_TRANSPORT_ENABLE_OPENSSL_IF_AVAILABLE, true);

        if(!SecuritySSLPlugin.OPENSSL_SUPPORTED && OpenSsl.isAvailable() && (settings.getAsBoolean(SSLConfigConstants.SECURITY_SSL_HTTP_ENABLE_OPENSSL_IF_AVAILABLE, true) || settings.getAsBoolean(SSLConfigConstants.SECURITY_SSL_TRANSPORT_ENABLE_OPENSSL_IF_AVAILABLE, true) )) {
            if (PlatformDependent.javaVersion() < 12) {
                ...
                log.warn("Support for OpenSSL with Java 12+ has been removed. Using JDK SSL instead.");
            }
        }
    ...
    private void initSSLConfig() {
        ...
        final boolean client = !"node".equals(this.settings.get(SecuritySSLPlugin.CLIENT_TYPE));
    ...
    public void initTransportSSLConfig() {
        ...
        if (useKeyStore) {
            ...
            if (settings.get(SSLConfigConstants.SECURITY_SSL_TRANSPORT_TRUSTSTORE_FILEPATH, null) == null) {
                throw new ElasticsearchException(SSLConfigConstants.SECURITY_SSL_TRANSPORT_TRUSTSTORE_FILEPATH
            ...
            try {
                ...
                if (extendedKeyUsageEnabled) {
                    ...
                    {
                        throw new ElasticsearchException(SSLConfigConstants.SECURITY_SSL_TRANSPORT_SERVER_KEYSTORE_ALIAS + ", "
            ...
            throw new ElasticsearchException(SSLConfigConstants.SECURITY_SSL_TRANSPORT_KEYSTORE_FILEPATH + " or "
     ...
     */
        ...
        if (useKeyStore) {
            ...
            if (settings.get(SSLConfigConstants.SECURITY_SSL_HTTP_KEYSTORE_FILEPATH, null) == null) {
                throw new ElasticsearchException(SSLConfigConstants.SECURITY_SSL_HTTP_KEYSTORE_FILEPATH
            ...
            if (httpClientAuthMode == ClientAuth.REQUIRE) {
                ...
                if (settings.get(SSLConfigConstants.SECURITY_SSL_HTTP_TRUSTSTORE_FILEPATH, null) == null) {
                    throw new ElasticsearchException(SSLConfigConstants.SECURITY_SSL_HTTP_TRUSTSTORE_FILEPATH
            ...
            throw new ElasticsearchException(SSLConfigConstants.SECURITY_SSL_HTTP_KEYSTORE_FILEPATH + " or "
    ...
    private void logOpenSSLInfos() {
        if (SecuritySSLPlugin.OPENSSL_SUPPORTED && OpenSsl.isAvailable()) {
    ...
    private void initEnabledSSLCiphers() {
        ...
        if(SecuritySSLPlugin.OPENSSL_SUPPORTED && OpenSsl.isAvailable() && OpenSsl.version() > 0x10101009L) {
        ...
        } else if(SecuritySSLPlugin.OPENSSL_SUPPORTED && OpenSsl.isAvailable()){
