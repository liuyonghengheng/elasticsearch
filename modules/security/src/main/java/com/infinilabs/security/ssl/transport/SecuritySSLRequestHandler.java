...
implements TransportRequestHandler<T> {
    ...
    public final void messageReceived(T request, TransportChannel channel, Task task) throws Exception {
        ...
        if(SSLRequestHelper.containsBadHeader(threadContext, "_security_ssl_")) {
        ...
        try {
            ...
            if (peerCerts != null
                ...
                if(threadContext != null) {
                    ...
                    threadContext.putTransient("_security_ssl_transport_principal", principal);
                    threadContext.putTransient("_security_ssl_transport_peer_certificates", x509PeerCerts);
                    threadContext.putTransient("_security_ssl_transport_local_certificates", x509LocalCerts);
                    threadContext.putTransient("_security_ssl_transport_protocol", sslhandler.engine().getSession().getProtocol());
                    threadContext.putTransient("_security_ssl_transport_cipher", sslhandler.engine().getSession().getCipherSuite());
                }
                messageReceivedDecorate(request, actualHandler, channel, task);
            } else {
