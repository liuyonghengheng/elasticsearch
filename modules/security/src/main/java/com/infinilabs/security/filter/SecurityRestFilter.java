...
import com.infinilabs.security.dlic.rest.api.WhitelistApiAction;
...
import com.infinilabs.security.ssl.util.SSLRequestHelper.SSLInfo;
...
public class SecurityRestFilter {
     ...
     * If whitelisting is enabled, then Non-SuperAdmin is allowed to access only those APIs that are whitelisted in requests
     ...
     * Further note: Some APIs are only accessible by SuperAdmin, regardless of whitelisting. For example: /_security/whitelist is only accessible by SuperAdmin.
    ...
    private boolean checkAndAuthenticateRequest(RestRequest request, RestChannel channel,
        ...
        try {
            if((sslInfo = SSLRequestHelper.getSSLInfo(settings, configPath, request, principalExtractor)) != null) {
                if(sslInfo.getPrincipal() != null) {
                    threadContext.putTransient("_security_ssl_principal", sslInfo.getPrincipal());
                }

                if(sslInfo.getX509Certs() != null) {
                     threadContext.putTransient("_security_ssl_peer_certificates", sslInfo.getX509Certs());
                }
                threadContext.putTransient("_security_ssl_protocol", sslInfo.getProtocol());
                threadContext.putTransient("_security_ssl_cipher", sslInfo.getCipher());
            }
        ...
        if(request.method() != Method.OPTIONS
                && !"/_security/health".equals(request.path())) {
