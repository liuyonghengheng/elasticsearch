...
import com.infinilabs.security.action.configupdate.ConfigUpdateNodeResponse;
...
import com.infinilabs.security.dlic.rest.validation.AbstractConfigurationValidator.ErrorType;
...
public abstract class AbstractApiAction extends BaseRestHandler {
	...
	/**
	 * Checks if it is valid to add role to roles or role_mapping.
	...
	protected boolean isValidRolesMapping(final RestChannel channel, final String role) {
		final SecurityDynamicConfiguration<?> rolesConfiguration = load(CType.ROLE, false);
		final SecurityDynamicConfiguration<?> rolesMappingConfiguration = load(CType.ROLE_MAPPING, false);
