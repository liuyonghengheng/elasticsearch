 ...
 * under the License.
 */
package org.elasticsearch.index.segmentscopy;
...
/**
 * copy related statistics, starting at the shard level and allowing aggregation to
 * indices and node level
 */
public class CopyStats implements ToXContentFragment, Writeable {
    ...
    public CopyStats(StreamInput in) throws IOException {
        currentAsSource.set(in.readVInt());
        currentAsTarget.set(in.readVInt());
        throttleTimeInNanos.set(in.readLong());
    }

    public void add(CopyStats copyStats) {
        if (copyStats != null) {
            this.currentAsSource.addAndGet(copyStats.currentAsSource());
            this.currentAsTarget.addAndGet(copyStats.currentAsTarget());
        }
        addTotals(copyStats);
    }

    public void addTotals(CopyStats copyStats) {
        if (copyStats != null) {
            this.throttleTimeInNanos.addAndGet(copyStats.throttleTime().nanos());
        }
    }
    ...
    public XContentBuilder toXContent(XContentBuilder builder, Params params) throws IOException {
        builder.startObject(Fields.COPY);
    ...
    public String toString() {
        return "copyStats, currentAsSource [" + currentAsSource() + "],currentAsTarget ["
