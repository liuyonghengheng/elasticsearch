...
/**
 * Keeps track of state related to shard copy.
 */
public class CopyState implements ToXContentFragment, Writeable {
...
//        TRANSLOG((byte) 3),
    ...
    public CopyState(ShardRouting shardRouting,
    ...
    public CopyState(StreamInput in) throws IOException {
        timer = new Timer(in);
    ...
    public synchronized CopyState setStage(Stage stage) {
        switch (stage) {
            case INIT:
    ...
    public static CopyState readRecoveryState(StreamInput in) throws IOException {
        return new CopyState(in);
    }
    ...
    public static class Translog extends Timer implements ToXContentFragment, Writeable {
         ...
         * <p>
         * A value of -1 ({@link CopyState.Translog#UNKNOWN} is return if this is unknown (typically a gateway recovery)
         */
