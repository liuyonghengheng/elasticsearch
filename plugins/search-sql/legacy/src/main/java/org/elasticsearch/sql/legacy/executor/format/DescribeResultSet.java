...
import org.elasticsearch.sql.legacy.executor.format.DataRows.Row;
import org.elasticsearch.sql.legacy.executor.format.Schema.Column;
import org.elasticsearch.sql.legacy.executor.format.Schema.Type;
    ...
    private List<Row> loadRows() {
        ...
        for (ObjectObjectCursor<String, ImmutableOpenMap<String, MappingMetadata>> indexCursor : indexMappings) {
            ...
            if (matchesPatternIfRegex(index, statement.getIndexPattern())) {
                ...
                // Assuming Elasticsearch 6.x, iterate through the only type of the index to get mapping data
