...
import static org.elasticsearch.sql.legacy.antlr.semantic.types.base.ElasticsearchDataType.UNKNOWN;

import com.google.common.collect.ImmutableList;
import java.util.List;
import java.util.Optional;
import java.util.Set;
import org.elasticsearch.sql.legacy.antlr.SimilarSymbols;
...
import org.elasticsearch.sql.legacy.antlr.semantic.types.function.AggregateFunction;
import org.elasticsearch.sql.legacy.antlr.semantic.types.function.ElasticsearchScalarFunction;
import org.elasticsearch.sql.legacy.antlr.semantic.types.function.ScalarFunction;
import org.elasticsearch.sql.legacy.antlr.semantic.types.operator.ComparisonOperator;
import org.elasticsearch.sql.legacy.antlr.semantic.types.operator.JoinOperator;
import org.elasticsearch.sql.legacy.antlr.semantic.types.operator.SetOperator;
...
public class TypeChecker implements GenericSqlParseTreeVisitor<Type> {
    ...
    public void visitRoot() {
        ...
        defineFunctionNames(ElasticsearchScalarFunction.values());
    ...
    public Type visitString(String text) {
        return ElasticsearchDataType.STRING;
    }

    @Override
    public Type visitInteger(String text) {
        return ElasticsearchDataType.INTEGER;
    }

    @Override
    public Type visitFloat(String text) {
        return ElasticsearchDataType.FLOAT;
    }

    @Override
    public Type visitBoolean(String text) {
        // "IS [NOT] MISSING" can be used on any data type
        return "MISSING".equalsIgnoreCase(text) ? UNKNOWN : ElasticsearchDataType.BOOLEAN;
    }

    @Override
    public Type visitDate(String text) {
        return ElasticsearchDataType.DATE;
    }
    ...
    public Type visitConvertedType(String text) {
        return ElasticsearchDataType.typeOf(text);
    }
