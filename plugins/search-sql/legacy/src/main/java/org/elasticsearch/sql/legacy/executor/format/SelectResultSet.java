...
import org.elasticsearch.sql.legacy.domain.Query;
import org.elasticsearch.sql.legacy.domain.Select;
import org.elasticsearch.sql.legacy.domain.TableOnJoinSelect;
import org.elasticsearch.sql.legacy.esdomain.mapping.FieldMapping;
...
public class SelectResultSet extends ResultSet {
    ...
    private void loadFromEsState(Query query) {
        ...
        if (indexMappings.containsKey(typeName)) {
            ...
            // Assuming Elasticsearch version 6.x, there can be only one type per index so this for loop should grab the only type
