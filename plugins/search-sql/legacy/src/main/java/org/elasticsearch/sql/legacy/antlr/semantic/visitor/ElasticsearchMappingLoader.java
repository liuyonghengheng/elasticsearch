...
package org.elasticsearch.sql.legacy.antlr.semantic.visitor;

import static org.elasticsearch.sql.legacy.antlr.semantic.types.base.ElasticsearchIndex.IndexType.INDEX;
import static org.elasticsearch.sql.legacy.antlr.semantic.types.base.ElasticsearchIndex.IndexType.NESTED_FIELD;

import java.util.Map;
import java.util.Set;
import java.util.stream.Collectors;
import org.elasticsearch.sql.legacy.antlr.semantic.scope.Environment;
import org.elasticsearch.sql.legacy.antlr.semantic.scope.Namespace;
import org.elasticsearch.sql.legacy.antlr.semantic.scope.SemanticContext;
import org.elasticsearch.sql.legacy.antlr.semantic.scope.Symbol;
...
import org.elasticsearch.sql.legacy.antlr.visitor.GenericSqlParseTreeVisitor;
import org.elasticsearch.sql.legacy.esdomain.LocalClusterState;
import org.elasticsearch.sql.legacy.esdomain.mapping.FieldMappings;
import org.elasticsearch.sql.legacy.esdomain.mapping.IndexMappings;
import org.elasticsearch.sql.legacy.utils.StringUtils;

/**
 * Load index and nested field mapping into semantic context
 */
public class ElasticsearchMappingLoader implements GenericSqlParseTreeVisitor<Type> {
    ...
    public ElasticsearchMappingLoader(SemanticContext context, LocalClusterState clusterState, int threshold) {
    ...
    public void visitAs(String alias, Type type) {
        if (!(type instanceof ElasticsearchIndex)) {
            return;
        }

        ElasticsearchIndex index = (ElasticsearchIndex) type;
    ...
    private void defineIndexType(String indexName) {
        environment().define(new Symbol(Namespace.FIELD_NAME, indexName), new ElasticsearchIndex(indexName, INDEX));
    }
    ...
    /**
     * Check if index name is NOT nested, for example. return true for index 'accounts' or '.elasticsearch_dashboards'
     *  but return false for nested field name 'a.projects'.
     */
    private boolean isNotNested(String indexName) {
        return indexName.indexOf('.', 1) == -1; // taking care of .elasticsearch_dashboards
    }
    ...
    private void defineFieldName(String fieldName, String type) {
        if ("NESTED".equalsIgnoreCase(type)) {
            defineFieldName(fieldName, new ElasticsearchIndex(fieldName, NESTED_FIELD));
        } else {
            defineFieldName(fieldName, ElasticsearchDataType.typeOf(type));
        }
    }
