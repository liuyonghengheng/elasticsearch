...
import static org.elasticsearch.sql.legacy.utils.StringUtils.toUpper;
 ...
 */
public enum ElasticsearchDataType implements BaseType {
     ...
     */
    private static final Map<String, ElasticsearchDataType> ALL_BASE_TYPES;
    static {
        ImmutableMap.Builder<String, ElasticsearchDataType> builder = new ImmutableMap.Builder<>();
        for (ElasticsearchDataType type : ElasticsearchDataType.values()) {
            builder.put(type.name(), type);
        }
        ALL_BASE_TYPES = builder.build();
    }

    public static ElasticsearchDataType typeOf(String str) {
        return ALL_BASE_TYPES.getOrDefault(toUpper(str), UNKNOWN);
    }

    /** Parent of current base type */
    private ElasticsearchDataType parent;

    ElasticsearchDataType(ElasticsearchDataType... compatibleTypes) {
        for (ElasticsearchDataType subType : compatibleTypes) {
            subType.parent = this;
        }
    }
    ...
    public boolean isCompatible(Type other) {
        ...
        if (!(other instanceof ElasticsearchDataType)) {
            // Nested data type is compatible with nested index type for type expression use
            if (other instanceof ElasticsearchIndex && ((ElasticsearchIndex) other).type() == NESTED_FIELD) {
        ...
        ElasticsearchDataType cur = (ElasticsearchDataType) other;
    ...
    public String toString() {
        return "Elasticsearch Data Type [" + getName() + "]";
    }
}
