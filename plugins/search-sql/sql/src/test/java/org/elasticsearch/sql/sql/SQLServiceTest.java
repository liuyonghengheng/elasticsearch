...
class SQLServiceTest {
  ...
  public void canExecuteSqlQuery() {
    ...
    sqlService.execute(
        new SQLQueryRequest(new JSONObject(), "SELECT 123", "_sql", "jdbc"),
  ...
  public void canExecuteCsvFormatRequest() {
    ...
    sqlService.execute(
        new SQLQueryRequest(new JSONObject(), "SELECT 123", "_sql", "csv"),
        new ResponseListener<QueryResponse>() {
          @Override
          public void onResponse(QueryResponse response) {
            assertNotNull(response);
  ...
  public void canCaptureErrorDuringExecution() {
    sqlService.execute(
        new SQLQueryRequest(new JSONObject(), "SELECT", "_sql", ""),
        new ResponseListener<QueryResponse>() {
          @Override
          public void onResponse(QueryResponse response) {
            fail();
