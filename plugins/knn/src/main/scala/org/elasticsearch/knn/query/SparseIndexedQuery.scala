...
import org.elasticsearch.knn.api.Vec
import org.elasticsearch.knn.models.{HashAndFreq, SparseIndexedSimilarityFunction}
import org.elasticsearch.knn.storage.UnsafeSerialization
  ...
  private val scoreFunction: java.util.function.Function[LeafReaderContext, MatchHashesAndScoreQuery.ScoreFunction] =
    (lrc: LeafReaderContext) => {
      ...
      (docId: Int, matchingTerms: Int) =>
        ...
        } else throw new ElasticknnRuntimeException(s"Couldn't advance to doc with id [$docId]")
    }
