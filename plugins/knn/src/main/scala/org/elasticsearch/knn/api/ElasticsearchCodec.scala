...
package org.elasticsearch.knn.api

import org.elasticsearch.knn.{ELASTICKNN_NAME, api}
import io.circe
...
private object Keys {
  ...
  val COSINE = "cosine"
  val DIMS = "dims"
  val EKNN_DENSE_FLOAT_VECTOR = s"${ELASTICKNN_NAME}_dense_float_vector"
  val EKNN_SPARSE_BOOL_VECTOR = s"${ELASTICKNN_NAME}_sparse_bool_vector"
...
object ElasticsearchCodec { esc =>
  ...
  implicit val mapping: ESC[Mapping] = new ESC[Mapping] {
    override def apply(t: Mapping): Json = t match {
      case m: Mapping.SparseBool => JsonObject(TYPE -> EKNN_SPARSE_BOOL_VECTOR, ELASTICKNN_NAME -> esc.encode(m))
      case m: Mapping.DenseFloat => JsonObject(TYPE -> EKNN_DENSE_FLOAT_VECTOR, ELASTICKNN_NAME -> esc.encode(m))
      case m: Mapping.SparseIndexed =>
        JsonObject(TYPE -> EKNN_SPARSE_BOOL_VECTOR, ELASTICKNN_NAME -> (esc.encode(m) ++ JsonObject(MODEL -> SPARSE_INDEXED)))
      case m: Mapping.JaccardLsh =>
        JsonObject(TYPE -> EKNN_SPARSE_BOOL_VECTOR, ELASTICKNN_NAME -> (esc.encode(m) ++ JsonObject(MODEL -> LSH, SIMILARITY -> JACCARD)))
      case m: Mapping.HammingLsh =>
        JsonObject(TYPE -> EKNN_SPARSE_BOOL_VECTOR, ELASTICKNN_NAME -> (esc.encode(m) ++ JsonObject(MODEL -> LSH, SIMILARITY -> HAMMING)))
      case m: Mapping.CosineLsh =>
        JsonObject(TYPE -> EKNN_DENSE_FLOAT_VECTOR, ELASTICKNN_NAME -> (esc.encode(m) ++ JsonObject(MODEL -> LSH, SIMILARITY -> ANGULAR)))
      case m: Mapping.L2Lsh =>
        JsonObject(TYPE -> EKNN_DENSE_FLOAT_VECTOR, ELASTICKNN_NAME -> (esc.encode(m) ++ JsonObject(MODEL -> LSH, SIMILARITY -> L2)))
      case m: Mapping.PermutationLsh =>
        JsonObject(TYPE -> EKNN_DENSE_FLOAT_VECTOR, ELASTICKNN_NAME -> (esc.encode(m) ++ JsonObject(MODEL -> PERMUTATION_LSH)))
    }

    override def apply(c: HCursor): Either[DecodingFailure, Mapping] =
      for {
        typ <- c.downField(TYPE).as[String]
        c <- c.downField(ELASTICKNN_NAME).as[Json].map(_.hcursor)
