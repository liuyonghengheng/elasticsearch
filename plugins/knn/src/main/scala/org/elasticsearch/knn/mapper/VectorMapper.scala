package org.elasticsearch.knn.mapper
...
import org.elasticsearch.knn.ElasticknnException.ElasticknnUnsupportedOperationException
import org.elasticsearch.knn.api.ElasticsearchCodec.nospaces
import org.elasticsearch.knn.{ELASTICKNN_NAME, ElasticknnException}
import org.elasticsearch.knn.api.{ElasticsearchCodec, JavaJsonMap, Mapping, Vec}
import org.elasticsearch.knn.models.Cache
import org.elasticsearch.knn.query.{ExactQuery, HashingQuery, SparseIndexedQuery}
...
object VectorMapper {
  ...
  val sparseBoolVector: VectorMapper[Vec.SparseBool] =
    new VectorMapper[Vec.SparseBool] {
      override val CONTENT_TYPE: String = s"${ELASTICKNN_NAME}_sparse_bool_vector"
      override def checkAndCreateFields(mapping: Mapping, field: String, vec: Vec.SparseBool): Try[Seq[IndexableField]] =
        if (mapping.dims != vec.totalIndices)
          Failure(ElasticknnException.vectorDimensions(vec.totalIndices, mapping.dims))
        else {
  ...
  val denseFloatVector: VectorMapper[Vec.DenseFloat] =
    new VectorMapper[Vec.DenseFloat] {
      override val CONTENT_TYPE: String = s"${ELASTICKNN_NAME}_dense_float_vector"
      override def checkAndCreateFields(mapping: Mapping, field: String, vec: Vec.DenseFloat): Try[Seq[IndexableField]] =
        if (mapping.dims != vec.values.length)
          Failure(ElasticknnException.vectorDimensions(vec.values.length, mapping.dims))
        else
          mapping match {
  ...
  class FieldType(typeName: String, fieldName: String, val mapping: Mapping)
    ...
    override def termQuery(value: Any, context: QueryShardContext): Query = value match {
      ...
      case _ =>
        throw new ElasticknnUnsupportedOperationException(
    ...
    override def valueFetcher(mapperService: MapperService, searchLookup: SearchLookup, format: String): ValueFetcher = {
      ...
      throw new ElasticknnUnsupportedOperationException(s"Field [${name()}] of type [${typeName()}] doesn't support this operation yet.")
    }
  }
...
abstract class VectorMapper[V <: Vec: ElasticsearchCodec] { self =>
  ...
  import org.elasticsearch.knn.utils.CirceUtils._
