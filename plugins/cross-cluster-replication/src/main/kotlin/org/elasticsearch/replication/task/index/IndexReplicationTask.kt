...
import org.elasticsearch.replication.repository.REMOTE_SNAPSHOT_NAME
...
import org.elasticsearch.replication.task.CrossClusterReplicationTask
...
import org.elasticsearch.replication.task.shard.ShardReplicationTask
import org.elasticsearch.replication.util.removeTask
...
import org.elasticsearch.replication.util.waitForNextChange
...
import org.elasticsearch.replication.ReplicationPlugin.Companion.REPLICATION_INDEX_TRANSLOG_PRUNING_ENABLED_SETTING
...
open class IndexReplicationTask(id: Long, type: String, action: String, description: String,
    ...
    private suspend fun pollShardTaskStatus(): IndexReplicationState {
        ...
        if (failedShardTasks.isNotEmpty()) {
            ...
            for ((shard, task) in failedShardTasks) {
                ...
                if (taskState is org.elasticsearch.replication.task.shard.FailedState) {
    ...
    private fun findAllReplicationFailedShardTasks(followerIndexName: String, clusterState: ClusterState)
        ...
        val failedShardTasks = persistentTasks.findTasks(ShardReplicationExecutor.TASK_NAME, Predicate { true }).stream()
                .filter { task -> task.state is org.elasticsearch.replication.task.shard.FailedState }
