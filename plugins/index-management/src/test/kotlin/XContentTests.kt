...
import org.elasticsearch.indexmanagement.indexstatemanagement.nonNullRandomConditions
import org.elasticsearch.indexmanagement.indexstatemanagement.randomAliasAction
import org.elasticsearch.indexmanagement.indexstatemanagement.randomAllocationActionConfig
import org.elasticsearch.indexmanagement.indexstatemanagement.randomChangePolicy
import org.elasticsearch.indexmanagement.indexstatemanagement.randomChannel
import org.elasticsearch.indexmanagement.indexstatemanagement.randomCloseActionConfig
import org.elasticsearch.indexmanagement.indexstatemanagement.randomDeleteActionConfig
import org.elasticsearch.indexmanagement.indexstatemanagement.randomDestination
import org.elasticsearch.indexmanagement.indexstatemanagement.randomForceMergeActionConfig
import org.elasticsearch.indexmanagement.indexstatemanagement.randomIndexPriorityActionConfig
import org.elasticsearch.indexmanagement.indexstatemanagement.randomManagedIndexConfig
//import org.elasticsearch.indexmanagement.indexstatemanagement.randomNotificationActionConfig
import org.elasticsearch.indexmanagement.indexstatemanagement.randomOpenActionConfig
import org.elasticsearch.indexmanagement.indexstatemanagement.randomPolicy
import org.elasticsearch.indexmanagement.indexstatemanagement.randomReadOnlyActionConfig
import org.elasticsearch.indexmanagement.indexstatemanagement.randomReadWriteActionConfig
import org.elasticsearch.indexmanagement.indexstatemanagement.randomReplicaCountActionConfig
import org.elasticsearch.indexmanagement.indexstatemanagement.randomRolloverActionConfig
import org.elasticsearch.indexmanagement.indexstatemanagement.randomShrinkAction
import org.elasticsearch.indexmanagement.indexstatemanagement.randomSnapshotActionConfig
import org.elasticsearch.indexmanagement.indexstatemanagement.randomState
import org.elasticsearch.indexmanagement.indexstatemanagement.randomTransition
import org.elasticsearch.indexmanagement.indexstatemanagement.toJsonString
import org.elasticsearch.indexmanagement.opensearchapi.convertToMap
...
class XContentTests : ElasticSearchTestCase() {

    fun `test policy parsing2`() {
        // val policyString = "{\"policy\":{\"states\":{\"hot\":{\"actions\":{\"rollover\":{\"max_doc_count\":100000000,\"max_primary_shard_size\":\"30gb\"},\"index_priority\":{\"priority\":100}}},\"cold\":{\"actions\":{\"index_priority\":{\"priority\":50}},\"min_age\":\"30d\"}}}}"
        val policyString = "{\n" +
                "  \"policy\": {\n" +
                "    \"phases\": {\n" +
                "      \"warm\": {\n" +
                ...
                "  }\n" +
                "}"
        // val policyString = "{\"policy\":{\"policy_id\":\"oIgOOTFUCg\",\"description\":\"FWWKkUtAFM\",\"last_updated_time\":1679972460228,\"schema_version\":995722996696712981,\"error_notification\":null,\"default_state\":\"BhZEiPKNsa\",\"states\":[{\"name\":\"BhZEiPKNsa\",\"actions\":[],\"transitions\":[]},{\"name\":\"bnIFGgJOoF\",\"actions\":[],\"transitions\":[]}],\"ism_template\":null,\"user\":null}}"
        println(policyString)
        val policy = parserWithType(policyString).parseWithType(id = "policy_id", parse = Policy.Companion::parse2).copy(lastUpdatedTime = Instant.now())
        for (state in policy.states) {
            println(state.toJsonString())
        }
        println("policy::: " + policy.toJsonString())
    }

     fun `test policy parsing Wait_for_snapshot`() {
        val policyString = "{\"policy\":{\"phases\":{\"delete\":{\"actions\":{\"wait_for_snapshot\":{\"policy\":\"slm-policy-name\"}}}}}}"
        println(policyString)
        val policy = parserWithType(policyString).parseWithType(id = "policy_id", parse = Policy.Companion::parse2).copy(lastUpdatedTime = Instant.now())
        for (state in policy.states) {
            println(state.toJsonString())
    ...
    fun `test delete action parsing`() {
       // val deleteAction = randomDeleteActionConfig()
        ...
        val deleteActionString = "{\"retry\":{\"count\":3,\"backoff\":\"exponential\",\"delay\":\"1m\"},\"delete\":{ \"delete_searchable_snapshot\": true}}"

        println(deleteActionString)
        val parsedDeleteAction = ISMActionsParser.instance.parse(parser(deleteActionString), 0)
        println(parsedDeleteAction.convertToMap())
