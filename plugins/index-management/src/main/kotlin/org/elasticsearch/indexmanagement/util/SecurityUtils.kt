...
class SecurityUtils {
    companion object {
        ...
        fun buildUser(threadContext: ThreadContext, requestedUser: User? = null): User? {
            ...
            val injectedUser: User? = User.parse(threadContext.getTransient<String>(ConfigConstants._SECURITY_USER_INFO_THREAD_CONTEXT))
...
//            }
        ...
        fun <T> userHasPermissionForResource(
            ...
            if (!userHasPermissionForResource(requestedUser, resourceUser, filterEnabled)) {
                actionListener.onFailure(
                    IndexManagementException.wrap(
                        ElasticsearchStatusException("Do not have permission for $resourceName [$resourceId]", RestStatus.FORBIDDEN)
                    )
                )
                return false
            }
         ...
         * string format used internally in the _SECURITY_USER_INFO_THREAD_CONTEXT and may be parsed using User.parse(<user string>).
         */
