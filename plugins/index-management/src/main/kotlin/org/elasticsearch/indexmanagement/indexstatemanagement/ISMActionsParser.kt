...
import org.elasticsearch.indexmanagement.indexstatemanagement.action.*
...
import org.elasticsearch.indexmanagement.spi.indexstatemanagement.model.ActionRetry
import org.elasticsearch.indexmanagement.spi.indexstatemanagement.model.ActionTimeout
...
class ISMActionsParser private constructor() {
    ...
    val parsers = mutableListOf(
        ...
        WaitForSnapshotPolicyActionParser()
    )
    ...
    fun parse2(xcp: XContentParser, totalActions: Int, type: String): Action {
        var action: Action? = null

        if (type == "set_priority") {
            action = parseAction(xcp, totalActions, "index_priority")
        } else if (type == "allocate") {
            action = parseAction(xcp, totalActions, "allocation")
        }
         else if (type == "forcemerge") {
            action = parseAction(xcp, totalActions, "force_merge")
        }
        else if (type == "readonly") {
            action = parseAction(xcp, totalActions, "read_only")
        }
        else {
            action = parseAction(xcp, totalActions, type)
        }
...
//        while (xcp.nextToken() != XContentParser.Token.END_OBJECT) {
//            val type = xcp.currentName()
//            println("type  $type  ${xcp.currentToken()}")
//            xcp.nextToken()
//            when (type) {
//                // ActionTimeout.TIMEOUT_FIELD -> timeout = ActionTimeout.parse(xcp)
//                // ActionRetry.RETRY_FIELD -> retry = ActionRetry.parse(xcp)
...
//                }
//                else -> {
//                    println("xcp  " + xcp.currentToken() + " " + xcp.currentName() + " " + xcp.contentType())
//                    action = parseAction(xcp, totalActions, type)
//                }
//            }
//        }
