...
package org.elasticsearch.indexmanagement.transform.opensearchapi
...
import org.elasticsearch.indexmanagement.opensearchapi.isRetryable
import org.elasticsearch.indexmanagement.transform.util.TransformLockManager
 ...
 * Message pattern: https://github.com/sohami/OpenSearch/blob/main/server/src/main/java/org/elasticsearch/action/support/TimeoutTaskCancellationUtility.java#L66
 */
 ...
 * if [block] throws an [ElasticsearchException] that is retriable (502, 503, 504 or 500 with message Time exceeded).
 *
 * If all retries fail the final exception will be rethrown. Exceptions caught during intermediate retries are
 * logged as warnings to [logger]. Similar to [org.elasticsearch.action.bulk.Retry], except these retries on
 ...
 */
fun isTransformOperationTimedOut(ex: ElasticsearchException): Boolean {
