...
data class State(
    ...
    init {
        ...
        if (hasDelete) require(transitions.isEmpty()) {
            "State=$name cannot contain transitions if using delete action"
        }
    }
...
//        }
    ...
    companion object {
        ...
        @JvmStatic
        @Throws(IOException::class)
        fun parse2(xcp: XContentParser, name0: String): State {
            var name: String? = null
            val actions: MutableList<Action> = mutableListOf()
            val transitions: MutableList<Transition> = mutableListOf()
            ...
            while (xcp.nextToken() != Token.END_OBJECT) {
                ...
                when (fieldName) {
                    ...
                    ACTIONS_FIELD -> {
                        ensureExpectedToken(Token.START_OBJECT, xcp.currentToken(), xcp)
                        while (xcp.nextToken() != Token.END_OBJECT) {
                            val actionName = xcp.currentName()
                            xcp.nextToken()
                            actions.add(ISMActionsParser.instance.parse2(xcp, actions.size, actionName))
                        }
                    }
//                    TRANSITIONS_FIELD -> {
//                        ensureExpectedToken(Token.START_ARRAY, xcp.currentToken(), xcp)
...
//                    }
                    MIN_AGE -> {
                        indexAge = TimeValue.parseTimeValue(xcp.text(), Conditions.MIN_INDEX_AGE_FIELD)
                    }
                    else -> throw IllegalArgumentException("Invalid field: [$fieldName] found in State.")
                }
                if (name0.isNotBlank()) {
