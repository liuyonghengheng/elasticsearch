...
import org.elasticsearch.indexmanagement.indexstatemanagement.IndexMetadataProvider.Companion.EVALUATION_FAILURE_MESSAGE
...
//                        }
        ...
        private fun onGetPolicyResponse(response: GetResponse) {
            if (!response.isExists || response.isSourceEmpty) {
                actionListener.onFailure(ElasticsearchStatusException("Could not find policy=${request.policyID}", RestStatus.NOT_FOUND))
                return
            }
            try {
        ...
        private fun createManagedIndices() {
            if (indicesToAdd.isNotEmpty()) {
                ...
                if (bulkReqTimeout < 0) {
                    actionListener.onFailure(ElasticsearchTimeoutException("Add policy API timed out after ClusterStateResponse"))
                    return
                }
