...
package org.elasticsearch.indexmanagement.indexstatemanagement.migration
...
import org.elasticsearch.indexmanagement.indexstatemanagement.util.updateISMTemplateRequest
...
class ISMTemplateService(
    ...
    suspend fun doMigration(timeStamp: Instant) {
        ...
        logger.debug("Doing ISM template migration ${++runTimeCounter} time.")
        cleanCache()

        lastUpdatedTime = timeStamp.minusSeconds(3600)
        logger.debug("Use $lastUpdatedTime as migrating ISM template last_updated_time")

        getIndexTemplates()
        logger.debug("ISM templates: $ismTemplateMap")

        getISMPolicies()
        logger.debug("Policies to update: ${policiesToUpdate.keys}")
