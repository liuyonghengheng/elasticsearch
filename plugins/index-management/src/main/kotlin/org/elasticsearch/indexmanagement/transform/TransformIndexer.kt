...
import org.elasticsearch.indexmanagement.transform.exceptions.TransformIndexException
        ...
        try {
            if (updatableDocsToIndex.isNotEmpty()) {
                ...
                backoffPolicy.retry(logger, listOf(RestStatus.TOO_MANY_REQUESTS)) {
                    ...
                    if (updatableDocsToIndex.isNotEmpty()) {
                        throw ExceptionsHelper.convertToElastic(retryableFailures.first().failure.cause)
                    }
                }
            }
